...
script_work_dir=$(pwd)
# Switch to the output directory to generate the platform specific wheels.
cd "${TFCI_OUTPUT_DIR}"
...
# Remove empty lines and comments from the wheel tags file. Then, read line by
# line to get the wheel tags.
grep -v '^#\|^$' "${TFCI_INSTALLER_WHL_TAGS_FILE}" | while IFS= read -r whl_tag
do
