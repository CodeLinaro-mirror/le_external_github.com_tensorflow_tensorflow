...
constexpr ElementCountLimit ElementCountLimitForKernel() {
  return kernel_type == kGenericOptimized ? ElementCountLimit::kInt
                                          : ElementCountLimit::kSizeT;
}

/// Validates that a tensor with elements has a data buffer.
TfLiteStatus ValidateTensorData(
  ...
  if (limit == ElementCountLimit::kInt) {
    TF_LITE_ENSURE_MSG(
        context, count <= static_cast<size_t>(std::numeric_limits<int>::max()),
        "Reduce tensor shape is invalid or size overflowed.");
  }
...
TfLiteStatus GetFlatSizeAsInt(TfLiteContext* absl_nonnull context,
  ...
  TF_LITE_ENSURE_MSG(
      context,
      CheckedNumElements(
          absl::MakeConstSpan(shape.DimsData(),
                              static_cast<size_t>(shape.DimensionsCount())),
          *flat_size) == kTfLiteOk,
...
TfLiteStatus EvalMean(TfLiteContext* context, TfLiteNode* node) {
  ...
  const TfLiteTensor* input = op_context.input;
  constexpr ElementCountLimit data_limit =
      ElementCountLimitForKernel<kernel_type>();
  TF_LITE_ENSURE_OK(context, ValidateTensorData(context, input, data_limit));
  TF_LITE_ENSURE_OK(context, ValidateTensorData(context, op_context.axis));
  TF_LITE_ENSURE_OK(context,
                    ValidateTensorData(context, op_context.output, data_limit));
...
TfLiteStatus EvalType(TfLiteContext* absl_nonnull context,
  ...
  TF_LITE_ENSURE_OK(context, ValidateTensorData(context, op_context->axis));
  TF_LITE_ENSURE_OK(
      context, ValidateTensorData(context, op_context->output, data_limit));
    ...
    switch (reduce_type) {
      case kSum:
        reducer = [](const T current, const T in) -> T {
          return WrappingAdd<T>(current, in);
        };
        init_value = T(0);
        break;
      case kProd:
        ...
        reducer = [](const T current, const T in) -> T {
          return WrappingMul<T>(current, in);
        };
        break;
      case kMax:
...
TfLiteStatus EvalSum(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (quantized) {
    ...
    constexpr ElementCountLimit data_limit =
        ElementCountLimitForKernel<kernel_type>();
    TF_LITE_ENSURE_OK(
        context, ValidateTensorData(context, op_context.input, data_limit));
    TF_LITE_ENSURE_OK(context, ValidateTensorData(context, op_context.axis));
    TF_LITE_ENSURE_OK(
        context, ValidateTensorData(context, op_context.output, data_limit));
...
TfLiteStatus EvalQuantizedProd(TfLiteContext* absl_nonnull context,
  ...
  TF_LITE_ENSURE_OK(context, ValidateTensorData(context, output, data_limit));
