...
void TestAuxInputWithDifferentSize(bool time_major, int n_batch,
  ...
  std::vector<float> lstm_input(sequence_length * n_batch * n_input, 1.0f);
  ...
  ASSERT_EQ(lstm.Invoke(), kTfLiteOk);
  EXPECT_EQ(lstm.GetFwOutput().size(), sequence_length * n_batch * n_output);
  EXPECT_EQ(lstm.GetBwOutput().size(), sequence_length * n_batch * n_output);
}
...
TEST_P(LSTMOpTest, BlackBoxTestWithAuxInputDifferentSize) {
  ...
  TestAuxInputWithDifferentSize(/*time_major=*/true, /*n_batch=*/1,
                                quantize_weights, asymmetric_quantize_inputs);
}

// Test aux_input in batch-major mode (time_major = false) with aux_input_size
// smaller than n_input.
TEST_P(LSTMOpTest, BlackBoxTestWithAuxInputBatchMajorDifferentSize) {
  const auto [quantize_weights, asymmetric_quantize_inputs] = GetParam();
  TestAuxInputWithDifferentSize(/*time_major=*/false, /*n_batch=*/2,
                                quantize_weights, asymmetric_quantize_inputs);
}
