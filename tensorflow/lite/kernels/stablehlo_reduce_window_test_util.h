...
inline std::vector<int64_t> DilateShape(std::vector<int64_t> shape,
  ...
  DCHECK_GE(dilations.size(), shape.size());
...
inline std::vector<int64_t> PadCropShape(std::vector<int64_t> shape,
  ...
  DCHECK_GE(padding.size(), 2 * shape.size());
  for (size_t i = 0; i < shape.size(); ++i) {
...
Tensor<T> Pad(const Tensor<T>& input, const std::vector<int64_t>& padding,
              const T padding_value) {
  DCHECK_LE(padding.size(), kMaxDims * 2);
  DCHECK_GE(padding.size(), 2 * input.shape.size());
...
Tensor<T> Crop(const Tensor<T>& input, const std::vector<int64_t>& cropping) {
  DCHECK_LE(cropping.size(), kMaxDims * 2);
  DCHECK_GE(cropping.size(), 2 * input.shape.size());
...
inline std::vector<int64_t> ReduceWindowShape(
  ...
  DCHECK_GE(base_dilations.size(), shape.size());
  DCHECK_GE(padding.size(), 2 * shape.size());
  DCHECK_GE(window_dimensions.size(), shape.size());
  DCHECK_GE(window_dilations.size(), shape.size());
  DCHECK_GE(window_strides.size(), shape.size());
