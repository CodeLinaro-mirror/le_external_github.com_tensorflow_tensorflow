...
inline TfLiteStatus LoadImageRaw(absl::string_view filename,
  ...
  if (raw_data.size() % kNumChannels != 0) {
    LOG(ERROR) << "Raw image size is not a multiple of " << kNumChannels;
    return kTfLiteError;
  }
...
inline void LoadImageJpeg(absl::string_view filename, ImageData* image_data) {
  // Reads image.
...
inline void Pad(ImageData* image_data, const PaddingParams& params) {
  ...
  std::fill_n(pad_params.left_padding, 4, 0);
  ...
  std::fill_n(pad_params.right_padding, 4, 0);
...
inline TfLiteStatus Normalize(ImageData* image_data,
  ...
  if (image_data->data.size() % kNumChannels != 0) {
    LOG(ERROR) << "Image data size is not a multiple of " << kNumChannels;
    return kTfLiteError;
  }
  ...
  for (const ImagePreprocessingStepParams& param : params.steps()) {
    if (param.has_cropping_params()) {
      ...
      if (!crop_params.has_cropping_fraction() &&
        ...
        LOG(ERROR) << "Cropping params must have either cropping_fraction or "
                      "target_size";
        return kTfLiteError;
      }
      if (crop_params.has_cropping_fraction() &&
      ...
      if (crop_params.has_target_size() &&
        ...
        LOG(ERROR) << "Invalid cropping target size";
        return kTfLiteError;
      }
      ...
      if (!resizing_params.has_target_size() ||
        ...
        LOG(ERROR) << "Invalid resizing target size";
        return kTfLiteError;
      }
      ...
      if (!padding_params.has_target_size() ||
        ...
        LOG(ERROR) << "Invalid padding target size";
        return kTfLiteError;
      }
    }
  }
...
TfLiteStatus ImagePreprocessingStage::Run() {
  ...
  for (const ImagePreprocessingStepParams& param : params.steps()) {
    if (param.has_cropping_params()) {
      ...
      if (Normalize(&image_data, param.normalization_params()) != kTfLiteOk) {
        return kTfLiteError;
      }
    }
  }
