...
bool MMapHandle::Map(const FileDescriptorView& fd, const size_t offset,
  ...
  XNNPACK_RETURN_CHECK(file_mapping_ != INVALID_HANDLE_VALUE,
                       "could not create a file mapping.");

  SYSTEM_INFO sys_info;
  GetSystemInfo(&sys_info);

  offset_page_adjustment_ = offset_ % sys_info.dwPageSize;
...
#elif defined(XNNPACK_CACHE_NO_MMAP_FOR_TEST)
  // This allocation is freed in UnMap and in the destructor.
  data_ = new uint8_t[size_];
  fd.SetPos(offset);
  XNNPACK_RETURN_CHECK(fd.Read(data_, size_),
