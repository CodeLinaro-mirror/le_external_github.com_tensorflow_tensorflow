...
TEST(IsCompatibleCacheFileTest, ReturnsTrueForACorrectHeader) {
  TempFileDesc fd;
  {
    ...
    write(fd.Value(), &header, sizeof(header));
    fd.Close();
  }
  EXPECT_TRUE(IsCompatibleCacheFile(fd.GetCPath()));
}

TEST(IsCompatibleCacheFileTest, ReturnsFalseForWrongHeaderVersion) {
  TempFileDesc fd;
  {
    XNNPackCacheHeader header{};
    header.version = XNNPackCacheHeader::kVersion + 1;
    memcpy(header.xnnpack_build_identifier,
           xnn_experimental_get_build_identifier_data(),
           xnn_experimental_get_build_identifier_size());
    write(fd.Value(), &header, sizeof(header));
  ...
  EXPECT_FALSE(IsCompatibleCacheFile(fd.GetCPath()));
}

TEST(IsCompatibleCacheFileTest, ReturnsFalseForWrongBuildIdentifier) {
  TempFileDesc fd;
  {
    XNNPackCacheHeader header{};
    header.version = XNNPackCacheHeader::kVersion;
    ...
    ASSERT_TRUE(fd.Write(&header, sizeof(header)));
    fd.Close();
  }
  EXPECT_FALSE(IsCompatibleCacheFile(fd.GetCPath()));
}
