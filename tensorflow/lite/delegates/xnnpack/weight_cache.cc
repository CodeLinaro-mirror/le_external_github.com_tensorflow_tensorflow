...
bool WeightCacheBuilder::StartBuildStep() {
  XNNPACK_RETURN_CHECK(IsStarted(),
                       "Trying to start a build step in an invavlid builder.")
  XNNPACK_RETURN_CHECK(
      is_build_step_.exchange(true),
      "Build step already started. This may be a concurrency issue.");
...
BufferLocation WeightCacheBuilder::Append(PackIdentifier pack_id,
  ...
  XNNPACK_ABORT_CHECK(is_build_step_.load(),
...
bool WeightCacheBuilder::StopBuildStep() {
  XNNPACK_RETURN_CHECK(!is_build_step_.load(),
