...
#include "third_party/XNNPACK/ynnpack/composites/composites.h"
#include "third_party/XNNPACK/ynnpack/include/ynnpack.h"
...
TfLiteStatus DefineConv(TfLiteContext* context, ynn_subgraph_t subgraph,
  ...
  // Transpose filter:
  // If groups == 1: [co, kh, kw, ci] -> [kh, kw, ci, co] (permutation: {1, 2,
  // 3, 0}) If groups > 1: [g, co/g, kh, kw, ci/g] -> [g, kh, kw, ci/g, co/g]
  ...
  if (is_quantized) {
    ...
    TF_LITE_ENSURE_STATUS(DefineQuantizedDot(
        ...
        (groups == 1) ? 6 : 8,  // rank_a
        (groups == 1) ? 4 : 5,  // rank_b
        a_reduce_axes, b_reduce_axes, current_input_id, a_scale_id, a_zp_id,
        transposed_filter_id, current_b_scale_id, current_b_zp_id,
        current_bias_id, (groups == 1) ? 1 : 3, out_scale_id, out_zp_id,
        is_per_channel, /*is_conv=*/true, GetYnnType(output_tensor.type),
        &dot_output_id));
  } else {
    TF_LITE_ENSURE_YNN_STATUS(
