...
class ReconfigBatchOpPass
 ...
 public:
  explicit ReconfigBatchOpPass(ReconfigBatchOpPassOptions options)
    ...
    global_prioritization_num_threads_ =
        options.global_prioritization_num_threads;
  }
  ReconfigBatchOpPass()
      : mlir::PassWrapper<ReconfigBatchOpPass,
                          mlir::OperationPass<mlir::ModuleOp>>() {}
 ...
 private:
  ...
  void runOnOperation() override {
    ...
    module.walk([&](mlir::TF::BatchFunctionOp batch_op) {
      ...
      if (global_prioritization_num_threads_ > 0) {
        batch_op.setNumBatchThreads(global_prioritization_num_threads_);
 ...
 protected:
  ...
  mlir::Pass::Option<int64_t> global_prioritization_num_threads_{
      *this, "tfrt-global-prioritization-num-threads", llvm::cl::init(0),
