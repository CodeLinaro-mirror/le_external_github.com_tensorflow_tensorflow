...
func.func @move_broadcast(%arg0: tensor<f32>) -> () {
  ...
  // CHECK:      {ici_weight_dist = true}
  // CHECK-NEXT: %[[SHAPE_0:.*]] = "tf.Const"()
  // CHECK:      {ici_weight_dist = true}
  // CHECK-NEXT: %[[FULL_0:.*]] = "tf.Fill"(%[[SHAPE_0]], %[[ELEM_0]]) {ici_weight_dist = true}
  // CHECK:      %[[ELEM_1:.*]] = "tf.Const"()
  // CHECK:      {ici_weight_dist = true}
  // CHECK-NEXT: %[[SHAPE_1:.*]] = "tf.Const"()
  // CHECK:      {ici_weight_dist = true}
  // CHECK-NEXT: %[[FULL_1:.*]] = "tf.Fill"(%[[SHAPE_1]], %[[ELEM_1]]) {ici_weight_dist = true}
  ...
  // CHECK-NEXT:     %[[IDINSIDE:.*]] = "tf.Identity"(%[[REPVAR]]) {ici_weight_dist = true} : (tensor<f32>) -> tensor<f32>
