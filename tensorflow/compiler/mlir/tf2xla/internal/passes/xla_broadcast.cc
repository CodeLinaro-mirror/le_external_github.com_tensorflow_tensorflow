...
constexpr std::string kICIWeightDist = "ici_weight_dist";
...
Value CreateZeroInput(Location loc, OpBuilder& builder, Attribute zero_attr,
  ...
  zero->setAttr(kICIWeightDist, builder.getBoolAttr(true));
  ConstOp shape = builder.create<ConstOp>(loc, shape_attr);
  shape->setAttr(kICIWeightDist, builder.getBoolAttr(true));
  FillOp fill = builder.create<FillOp>(loc, shape, zero);
  fill->setAttr(kICIWeightDist, builder.getBoolAttr(true));
  return fill;
}
...
LogicalResult MoveBroadcastToCluster(OpBuilder& builder,
  ...
  assigned_id->setAttr(kICIWeightDist,
