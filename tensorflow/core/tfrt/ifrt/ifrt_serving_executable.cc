...
// is expected to be a prefix of the reshaped shape in case of dynamic DMA.
// Currently, only flattening to 1D is supported.
absl::StatusOr<tensorflow::Tensor> MaybeReshapeInputForDynamicDma(
    const tensorflow::Tensor& input,
    const tensorflow::TensorShape& reshaped_shape) {
  if (input.dims() > 2 || reshaped_shape.dims() > 2) {
    return absl::UnimplementedError(absl::StrCat(
        "MaybeReshapeInputForDynamicDma only supports input and reshaped "
    ...
    if (variable_arg_index < variable_arg_indices.size() &&
      ...
      if (reshaped.shape() != reshaped_shape) {
        if (dtypes_and_shapes[i].static_shape.has_value()) {
          TF_ASSIGN_OR_RETURN(reshaped, MaybeReshapeInputForDynamicDma(
