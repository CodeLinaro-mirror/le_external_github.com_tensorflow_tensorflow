  ...
  for (const int i : variable_arg_indices) {
    ...
    *sharding_config.mutable_sharding() =
    ...
    for (const auto& device : devices) {
      sharding_config.add_device_ids(device->global_device_id().value());
    }

    TF_RETURN_IF_ERROR(ifrt_serving::LoadRestoredTensorAsIfrtLoadedVariable(
        runtime_name, ifrt_client_, thread_pool_, ifrt_restore_tensor_registry_,
        ifrt_loaded_variable_registry_, checkpoint_loader_queue_,
        sharding_config));
  }
