...
absl::Status MlrtIfrtRestoreVariableKernel::InvokeHelper() {
  std::optional<ifrt_serving::CheckpointLoader*> checkpoint_loader =
      context().resource_context().GetResource<ifrt_serving::CheckpointLoader>(
          ifrt_serving::kIfrtCheckpointLoaderName);
  if (!checkpoint_loader.has_value()) {
    return absl::InternalError("Did not find IfrtCheckpointLoader resource.");
  }
  if (*checkpoint_loader == nullptr) {
    return absl::InternalError("IfrtCheckpointLoader must not be null.");
  }
  return (*checkpoint_loader)
      ->Load(prefix(), var_handles(), tensor_names(), shape_and_slices(),
             restored_dtypes(), truncate_in_cast(), context());
}
