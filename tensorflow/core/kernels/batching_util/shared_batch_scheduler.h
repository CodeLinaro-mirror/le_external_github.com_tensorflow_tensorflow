...
class SharedBatchScheduler
 ...
 public:
  ...
  struct QueueOptions {
    ...
    // If true, the batch scheduler lazily filters tasks whose RPC deadline has
    // expired or were cancelled before they are sent for execution.
...
class PriorityTaskQueue {
 public:
  ...
  std::vector<std::unique_ptr<TaskType>> RemoveTask(int size) {
    ...
    // Invoke absl::Now() out side the loop to avoid repeated calls.
...
Queue<TaskType>::Queue(
    ...
    : tasks_priority_queue_(
          ...
          options.enable_lazy_cancellation_filtering,
