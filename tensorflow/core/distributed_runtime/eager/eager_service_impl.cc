...
absl::Status GetNumRetvals(
  ...
  *ndef.mutable_attr() = attrs;
  ...
  TF_RETURN_IF_ERROR(
      InOutTypesForNode(ndef, *op_def, &input_types, &output_types));
  if (output_types.size() > std::numeric_limits<int>::max()) {
    return absl::InternalError(
        absl::StrCat("Output size is too large: ", output_types.size()));
  }
