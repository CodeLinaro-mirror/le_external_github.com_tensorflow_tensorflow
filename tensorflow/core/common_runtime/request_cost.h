...
class RequestCost {
 public:
  ...
  // It's thread-safe, and can be called from different threads.
  void RecordMetrics(
      const std::vector<std::pair<absl::string_view, float>>& metrics);
  ...
  absl::flat_hash_map<std::string, float> GetMetrics() const;
 ...
 private:
  ...
  absl::flat_hash_map<std::string, float> metric_map_ ABSL_GUARDED_BY(mutex_);
