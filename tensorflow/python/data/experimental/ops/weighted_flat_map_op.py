...
def _weighted_flat_map(  # pylint: disable=unused-private-name
  ...
  input dataset. All elements from a dataset are read before reading the
  elements from the next dataset.
  ...
  Returns:
    A dataset that reads elements from all its inputs, exhaustively reading the
    elements from an input before proceeding to the next input. The number of
    elements read from an input is in proportion to its weight given in
    `weights`.
  """
