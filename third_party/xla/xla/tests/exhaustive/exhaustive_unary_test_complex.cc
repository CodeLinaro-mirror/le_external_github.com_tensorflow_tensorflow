...
UNARY_TEST_COMPLEX_64(Log, {
  ...
  ErrorSpecGen error_spec_gen = +[](complex64 x) {
    ...
    return ErrorSpec{std::numeric_limits<float>::epsilon(),
                     50 * std::numeric_limits<float>::epsilon()};
  };
...
UNARY_TEST_COMPLEX_64(Sqrt, {
  ...
  ErrorSpecGen error_spec_gen = +[](complex64 x) {
    ...
    return ErrorSpec{std::sqrt(std::numeric_limits<float>::denorm_min()),
                     50 * std::numeric_limits<float>::epsilon()};
  };
...
UNARY_TEST_COMPLEX_64(Rsqrt, {
  ...
  ErrorSpecGen error_spec_gen = +[](complex64 x) {
    ...
    if (std::abs(x) < norm_min) {
      ...
      return ErrorSpec(std::sqrt(std::numeric_limits<float>::min()),
                       10 * denorm_min / std::abs(x));
    }
    return ErrorSpec{std::sqrt(std::numeric_limits<float>::min()),
                     50 * std::numeric_limits<float>::epsilon()};
  };
...
UNARY_TEST_COMPLEX_128(Sqrt, {
  ErrorSpecGen error_spec_gen = +[](complex128 x) {
    ...
    return ErrorSpec{std::sqrt(std::numeric_limits<double>::denorm_min()),
...
UNARY_TEST_COMPLEX_128(Rsqrt, {
  ErrorSpecGen error_spec_gen = +[](complex128 x) {
    ...
    if (std::abs(x) < norm_min) {
      // Gradually loosen the relative tolerance as abs(x) becomes smaller
      // than norm_min, letting it reach 100% when abs(x) = 10 * denorm_min.
      return ErrorSpec(std::sqrt(std::numeric_limits<double>::min()),
                       10 * denorm_min / std::abs(x));
    }
    return ErrorSpec{std::sqrt(std::numeric_limits<double>::min()),
                     50 * std::numeric_limits<double>::epsilon()};
...
UNARY_TEST_COMPLEX_128(Tanh, {
  ErrorSpecGen error_spec_gen = [](complex128 x) {
    ...
    // float accuracy here.
    return ErrorSpec{2 * std::numeric_limits<double>::min(), 2e-8};
  };
