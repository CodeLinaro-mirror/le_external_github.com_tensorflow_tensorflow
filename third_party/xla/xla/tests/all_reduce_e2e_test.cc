...
HloOpcode ReductionKindToOpcode(PrimitiveType element_type,
                                ReductionKind reduction_kind) {
  if (element_type == PRED) {
    if (reduction_kind == ReductionKind::MIN) {
      return HloOpcode::kAnd;
    }
    if (reduction_kind == ReductionKind::MAX) {
      return HloOpcode::kOr;
    }
  }
  return ReductionKindToOpcode(reduction_kind).value();
}
...
TEST_P(AllReduceTypesTest, SupportedTypes2GPUs) {
  ...
  const HloOpcode opcode = ReductionKindToOpcode(element_type, reduction_kind);
