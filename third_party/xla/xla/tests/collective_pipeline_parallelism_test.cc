...
StatusOr<Literal> CreateLinearLayerWeights(int64_t size, float factor) {
...
StatusOr<Literal> CreateZeroInputR2(int64_t microbatches, int64_t size) {
  return LiteralUtil::CreateLiteralWithGenerator<F32, float>(
      ShapeUtil::MakeShape(F32, {microbatches, size}),
      [&](absl::Span<const int64_t> idx) -> float { return 0.0; });
};
StatusOr<Literal> CreateFingerprintInput(int64_t microbatches, int64_t size,
                                         float factor = 1.0) {
  return LiteralUtil::CreateLiteralWithGenerator<F32, float>(
      ShapeUtil::MakeShape(F32, {microbatches, size}),
      [&](absl::Span<const int64_t> idx) -> float {
