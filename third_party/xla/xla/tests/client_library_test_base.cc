...
absl::Status ClientLibraryTestBase::ComputeAndCompareLiteralWithStatus(
  ...
  Shape layout_shape;
  if (test_type_.has_value() && test_type_.value() != F32) {
      ...
      ShapeUtil::ForEachMutableSubshape(
          &layout_shape, [&](Shape* subshape, const ShapeIndex& /*index*/) {
            if (subshape->element_type() == F32) {
              subshape->set_element_type(test_type_.value());
            }
          });
...
XlaComputation ClientLibraryTestBase::CreateScalarMax() {
  ...
  auto shape = ShapeUtil::MakeShape(test_type_.value_or(F32), {});
...
Shape ClientLibraryTestBase::MaybeConvertShapeToTestType(const Shape& shape) {
  ...
  ShapeUtil::ForEachMutableSubshape(
      &new_shape,
      [test_type = test_type_.value()](Shape* subshape, const ShapeIndex&) {
        if (subshape->element_type() == F32) {
...
Literal ClientLibraryTestBase::MaybeConvertLiteralToTestType(
  ...
  if (!test_type_.has_value()) {
    return literal.Clone();
  }
  switch (test_type_.value()) {
    case BF16:
    ...
    default:
      LOG(FATAL) << "Unsupported test type: " << test_type_.value();
  }
}
