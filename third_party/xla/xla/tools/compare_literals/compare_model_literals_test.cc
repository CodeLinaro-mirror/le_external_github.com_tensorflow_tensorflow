...
class CompareModelLiteralsTest : public ::testing::Test {
 protected:
  void SetUp() override {
    golden_dir_ = tsl::io::JoinPath(testing::TempDir(), "golden_dir");
    test_dir_ = tsl::io::JoinPath(testing::TempDir(), "test_dir");
...
TEST_F(CompareModelLiteralsTest, ComparesMultipleLiteralsAndDevicesCorrectly) {
  ...
  ASSERT_THAT(result.entries, SizeIs(2));

  // Verify literal_0
  const ModelLiteralEntry& entry0 = result.entries[0];
  EXPECT_THAT(entry0.literal_index, Eq(0));
  EXPECT_THAT(entry0.literal_name, Eq("literal_0"));
  EXPECT_THAT(entry0.num_devices, Eq(2));
  EXPECT_DOUBLE_EQ(entry0.min_exact_match_pct, 100.0);
  EXPECT_DOUBLE_EQ(entry0.max_abs_error, 0.0);
  EXPECT_DOUBLE_EQ(entry0.max_rel_error, 0.0);

  // Verify literal_1
  const ModelLiteralEntry& entry1 = result.entries[1];
  EXPECT_THAT(entry1.literal_index, Eq(1));
  EXPECT_THAT(entry1.num_devices, Eq(2));
  EXPECT_DOUBLE_EQ(entry1.min_exact_match_pct, 50.0);  // 1/2 match on device 1
  EXPECT_NEAR(entry1.max_abs_error, 0.05, 1e-4);
  ASSERT_THAT(entry1.per_device_results, SizeIs(2));
  EXPECT_DOUBLE_EQ(entry1.per_device_results[0].exact_match_pct, 100.0);
  EXPECT_DOUBLE_EQ(entry1.per_device_results[1].exact_match_pct, 50.0);
}
...
TEST_F(CompareModelLiteralsTest, HandlesAsymmetricFilesGracefully) {
  ...
  EXPECT_THAT(result.entries[0].literal_index, Eq(0));
  EXPECT_THAT(result.entries[0].num_devices, Eq(1));
...
TEST_F(CompareModelLiteralsTest, HandlesMismatchedShapesGracefully) {
  ...
  ASSERT_OK_AND_ASSIGN(ModelComparisonResult result,
                       CompareModelDirectories(golden_dir_, test_dir_));
  ASSERT_THAT(result.entries, SizeIs(1));
  const ModelLiteralEntry& entry = result.entries[0];
  EXPECT_THAT(entry.failed_devices, Eq(1));
  EXPECT_DOUBLE_EQ(entry.min_exact_match_pct, 0.0);
  ASSERT_THAT(entry.per_device_results, SizeIs(1));
  EXPECT_FALSE(entry.per_device_results[0].comparison_ok);
  EXPECT_THAT(entry.per_device_results[0].error_message, HasSubstr("Shapes"));
...
TEST_F(CompareModelLiteralsTest, AggregatesAnomaliesAcrossThreeDevices) {
  ...
  EXPECT_THAT(entry.total_nan_mismatches, Eq(1));
  EXPECT_DOUBLE_EQ(entry.min_exact_match_pct, 50.0);
  EXPECT_TRUE(std::isinf(entry.max_abs_error));
  ASSERT_THAT(entry.per_device_results, SizeIs(3));
  EXPECT_NEAR(entry.per_device_results[1].max_abs_error, 0.1, 1e-4);
  EXPECT_TRUE(std::isinf(entry.per_device_results[2].max_abs_error));
}
...
TEST_F(CompareModelLiteralsTest, JsonAndTsvFormattingAndWriting) {
  ...
  const std::string json_path = tsl::io::JoinPath(test_dir_, "out.json");
  const std::string tsv_path = tsl::io::JoinPath(test_dir_, "out.tsv");
  const std::string dev_tsv_path = tsl::io::JoinPath(test_dir_, "out_dev.tsv");
