...
#include "third_party/jsoncpp/include/json/forwards.h"
#include "json/json.h"
...
MATCHER_P(JsonArraySizeIs, expected_size, "") {
  return arg.isArray() &&
         arg.size() == static_cast<Json::ArrayIndex>(expected_size);
}
...
TEST_F(GenerateBenchmarkMatricesTest,
  ...
  // Refined expectation: gemma_test/GPU_L4 and gemma_test/CPU_X86
  EXPECT_THAT(matrix, JsonArraySizeIs(2));
...
TEST_F(GenerateBenchmarkMatricesTest,
  ...
  EXPECT_THAT(matrix, JsonArraySizeIs(1));
