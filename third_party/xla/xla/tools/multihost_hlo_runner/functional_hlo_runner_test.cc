...
void CompileAndFilecheck(
  ...
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<xla::PjRtClient> client,
                          GetPjRtClient());
  FunctionalHloRunner::RawCompileOptions raw_opts;
  raw_opts.num_partitions = num_partitions;
  raw_opts.xla_dump_to = dump_dir;
  TF_EXPECT_OK(FunctionalHloRunner::LoadAndCompile(
      *client, xla::DebugOptions{}, preproc_options, raw_opts, hlo_file,
      InputFormat::kText));
