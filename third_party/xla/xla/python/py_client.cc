...
/* static */ absl::StatusOr<nb_class_ptr<PyLoadedExecutable>> PyClient::Compile(
  ...
  if (options.executable_build_options.debug_options().xla_use_shardy()) {
    ...
    TF_RETURN_IF_ERROR(tsl::StatusScopedDiagnosticHandler(mlirContext)
                           .consumeStatus(pm.run(*module)));
  }
