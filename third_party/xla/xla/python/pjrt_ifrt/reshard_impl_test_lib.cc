...
TEST_P(ReshardTest, DifferentSourceLayout) {
  ...
  // layout in order to ensure the test coverage.
  TF_ASSERT_OK_AND_ASSIGN(const auto actual_src_layout,
                          src_array->pjrt_layout());
  ...
  TF_ASSERT_OK_AND_ASSIGN(std::shared_ptr<const xla::PjRtLayout> dst_layout,
                          dst_array->pjrt_layout());
  if (dst_layout == nullptr) {
    TF_ASSERT_OK_AND_ASSIGN(
        dst_layout, client_->GetDefaultPjRtLayout(
