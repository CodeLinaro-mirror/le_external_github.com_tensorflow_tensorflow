...
TEST(XlaBuilderTest, SetAndGetSharding) {
  ...
  absl::StatusOr<std::optional<OpSharding>> p0_sharding = b.GetOpSharding(p0);
  EXPECT_OK(p0_sharding.status());
  EXPECT_TRUE(p0_sharding->has_value());
  EXPECT_EQ(HloSharding::FromProto(p0_sharding->value()).value(),
            hlo_sharding_1);

  EXPECT_OK(b.SetInstructionSharding(p0, std::nullopt));
  p0_sharding = b.GetOpSharding(p0);
  EXPECT_OK(p0_sharding.status());
  EXPECT_FALSE(p0_sharding->has_value());

  EXPECT_OK(b.SetInstructionSharding(p0, op_sharding_2));
  p0_sharding = b.GetOpSharding(p0);
  EXPECT_OK(p0_sharding.status());
  EXPECT_TRUE(p0_sharding->has_value());
  EXPECT_EQ(HloSharding::FromProto(p0_sharding->value()).value(),
