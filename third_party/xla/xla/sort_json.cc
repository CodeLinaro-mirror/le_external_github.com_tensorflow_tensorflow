...
void SkipWhitespace(absl::string_view json, int& index) {
...
absl::Status EnsureWithinBounds(absl::string_view json, int index,
...
absl::Status Consume(absl::string_view json, int& index, char c,
                     bool optional = false) {
  SkipWhitespace(json, index);
  TF_RETURN_IF_ERROR(EnsureWithinBounds(json, index, std::string(1, c)));
  if (json[index] == c) {
    index++;
    SkipWhitespace(json, index);
  } else if (!optional) {
...
absl::StatusOr<std::unique_ptr<T>> ParseSequence(absl::string_view outer_json,
                                                 int& index, ElemFn elem_fn) {
  TF_RETURN_IF_ERROR(Consume(outer_json, index, begin));
  TF_RETURN_IF_ERROR(EnsureWithinBounds(outer_json, index, name));
...
bool HandleEscape(absl::string_view outer_json, int& index, bool& is_escaped) {
  if (is_escaped) {
    is_escaped = false;
    index++;
    return true;
...
bool LiteralIsFinished(absl::string_view outer_json, int& index,
...
absl::StatusOr<absl::string_view> ParseLiteral(absl::string_view outer_json,
  ...
  TF_RETURN_IF_ERROR(EnsureWithinBounds(outer_json, index, "literal"));
...
absl::StatusOr<JsonField> ParseField(absl::string_view outer_json, int& index);

absl::StatusOr<JsonValue> ParseValue(absl::string_view outer_json, int& index) {
  JsonValue value;
  SkipWhitespace(outer_json, index);
  TF_RETURN_IF_ERROR(EnsureWithinBounds(outer_json, index, "value"));
  auto c = outer_json[index];
  if (c == '{') {
    constexpr static char kObject[] = "object\0";
    auto seq = ParseSequence<JsonObject, '{', '}', kObject>(outer_json, index,
                                                            ParseField);
    TF_ASSIGN_OR_RETURN(value, std::move(seq));
  } else if (c == '[') {
    constexpr static char kArray[] = "array\0";
...
void Serialize(const JsonSequence<ElemT>& object, std::string* result) {
  ...
  absl::StrAppend(result, std::string(1, begin_brace));
  bool has_preceeding = false;
  for (const auto& elem : elems) {
    if (has_preceeding) {
      absl::StrAppend(result, ",");
    }
    absl::StrAppend(result, elem);
    has_preceeding = true;
  }
  absl::StrAppend(result, std::string(1, end_brace));
}
