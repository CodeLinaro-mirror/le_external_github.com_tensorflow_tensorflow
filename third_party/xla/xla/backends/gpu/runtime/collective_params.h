...
enum class SymmetricMemoryType {
  ...
  // CANNOT be
  // used on standard HLO buffers (which may be aliased or reused for other
  // operations,
  // making the initial pointer exchange invalid). It must ONLY be used for
  // buffers
  // allocated by the thunk outside of XLA, such as scratch buffers.
  kXlaRendezvous = 2
};
...
struct KernelArgDescriptor {
  ...
  // buffer arguments. For example, if there are two IO buffers and 4 scratch
  // buffers, the valid indices for IO buffers are 0, 1 and for scratch buffers
  // are 0, 1, 2, 3.
...
struct CollectiveKernelSpec {
  // Specs for input/output buffers.
  std::vector<IoBufferSpec> io_buffers;
