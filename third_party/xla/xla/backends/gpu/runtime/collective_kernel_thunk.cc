...
absl::StatusOr<std::vector<se::KernelArg>> BuildKernelArguments(
    ...
    const Thunk::ExecuteParams& params, const RankId rank,
    const uint32_t invocation_count, const se::DeviceAddressBase metadata,
    const GpuCliqueKey& clique_key, const size_t num_parameters) {
  ...
  const int32_t param_index_offset = [](const CollectiveKernelSpec& spec) {
    ...
    // allocations.
    for (const auto& op : spec.operand_buffer_specs) {
      if (op.requires_multimem) {
        param_index++;
      }
    }
    for (const auto& res : spec.result_buffer_specs) {
  ...
  for (const KernelArgDescriptor& desc : kernel_spec.argument_descriptors) {
    switch (desc.type) {
      case KernelArgType::kInputBuffer: {
        ASSIGN_OR_RETURN(int32_t buffer_index,
...
absl::StatusOr<bool> CollectiveKernelThunk::IsSupported(
  ...
  if (kernel_name_.empty() || kernel_spec_.argument_descriptors.empty()) {
    return false;
  }
...
absl::Status CollectiveKernelThunk::Prepare(const PrepareParams& params) {
  ...
  TF_RET_CHECK(kernel_spec_.operand_buffer_specs.size() == buffers_.size())
      << "Kernel spec operand_buffer_specs size ("
      << kernel_spec_.operand_buffer_specs.size()
      << ") must equal thunk buffers size (" << buffers_.size() << ")";
  TF_RET_CHECK(kernel_spec_.result_buffer_specs.size() == buffers_.size())
      << "Kernel spec result_buffer_specs size ("
      << kernel_spec_.result_buffer_specs.size()
      << ") must equal thunk buffers size (" << buffers_.size() << ")";
  ...
  if (!per_stream_memory_.contains(params.executor)) {
    ...
    for (size_t i = 0; i < kernel_spec_.operand_buffer_specs.size(); ++i) {
      if (kernel_spec_.operand_buffer_specs[i].symmetric_memory_type ==
    ...
    for (size_t i = 0; i < kernel_spec_.result_buffer_specs.size(); ++i) {
      if (kernel_spec_.result_buffer_specs[i].symmetric_memory_type ==
          SymmetricMemoryType::kLoadStoreAccessible) {
        RETURN_IF_ERROR(
            params.collective_memory_requests->RequestSymmetricAllocation(
...
absl::Status CollectiveKernelThunk::Initialize(const InitializeParams& params) {
  ...
  if (state != nullptr) {
    ...
    for (size_t i = 0; i < kernel_spec_.operand_buffer_specs.size(); ++i) {
      if (kernel_spec_.operand_buffer_specs[i].requires_multimem) {
        parameters.push_back(params.buffer_allocations->GetDeviceAddress(
            buffers_[i].source_buffer.slice));
      }
    }
    for (size_t i = 0; i < kernel_spec_.result_buffer_specs.size(); ++i) {
      if (kernel_spec_.result_buffer_specs[i].requires_multimem) {
        parameters.push_back(params.buffer_allocations->GetDeviceAddress(
    ...
    if (is_multimem_enabled() && params.collective_memory != nullptr) {
    ...
    CollectiveKernelMetadata metadata_struct;
    metadata_struct.rank = state->rank.value();
    RETURN_IF_ERROR(CopyCollectiveMetadataToDevice(
        params.stream, metadata_struct, param_to_peers_ptrs, multimem_addresses,
        state->metadata));
    return absl::OkStatus();
  }
...
absl::Status CollectiveKernelThunk::ExecuteOnStream(
  ...
  for (const auto& op : kernel_spec_.operand_buffer_specs) {
    if (op.requires_multimem) {
      num_parameters++;
    }
  }
  for (const auto& res : kernel_spec_.result_buffer_specs) {
    if (res.requires_multimem) {
      num_parameters++;
    }
  }
  num_parameters += kernel_spec_.scratch_buffers.size();
...
CollectiveKernelThunk::FromProto(
  ...
  if (thunk_proto.has_kernel_spec()) {
    const auto& proto_spec = thunk_proto.kernel_spec();
    kernel_spec.operand_buffer_specs.reserve(
        proto_spec.operand_buffer_specs_size());
    for (const auto& io : proto_spec.operand_buffer_specs()) {
      TF_RET_CHECK(io.symmetric_memory_type() >= 0 &&
                   io.symmetric_memory_type() <= 2)
          << "Invalid symmetric_memory_type: " << io.symmetric_memory_type();
      kernel_spec.operand_buffer_specs.push_back(
          {io.requires_multimem(),
           static_cast<SymmetricMemoryType>(io.symmetric_memory_type())});
    }
    kernel_spec.result_buffer_specs.reserve(
        proto_spec.result_buffer_specs_size());
    for (const auto& io : proto_spec.result_buffer_specs()) {
      TF_RET_CHECK(io.symmetric_memory_type() >= 0 &&
                   io.symmetric_memory_type() <= 2)
          << "Invalid symmetric_memory_type: " << io.symmetric_memory_type();
      kernel_spec.result_buffer_specs.push_back(
          {io.requires_multimem(),
           static_cast<SymmetricMemoryType>(io.symmetric_memory_type())});
    }
    kernel_spec.scratch_buffers.reserve(proto_spec.scratch_buffers_size());
    for (const auto& scratch : proto_spec.scratch_buffers()) {
      TF_RET_CHECK(scratch.symmetric_memory_type() >= 0 &&
                   scratch.symmetric_memory_type() <= 2)
    ...
    for (const auto& arg : proto_spec.argument_descriptors()) {
      TF_RET_CHECK(arg.type() >= 0 && arg.type() <= kNumKernelArgTypes)
...
absl::StatusOr<ThunkProto> CollectiveKernelThunk::ToProto() const {
  ...
  *thunk_proto->mutable_collective_config() = collective_config_.ToProto();
  ...
  proto_spec->mutable_operand_buffer_specs()->Reserve(
      kernel_spec_.operand_buffer_specs.size());
  for (const auto& op : kernel_spec_.operand_buffer_specs) {
    auto* io_proto = proto_spec->add_operand_buffer_specs();
  ...
  proto_spec->mutable_result_buffer_specs()->Reserve(
      kernel_spec_.result_buffer_specs.size());
  for (const auto& res : kernel_spec_.result_buffer_specs) {
    auto* io_proto = proto_spec->add_result_buffer_specs();
