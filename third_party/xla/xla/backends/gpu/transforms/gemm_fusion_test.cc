...
ENTRY main {
  p_lhs = s8[2048,64,5334]{2,1,0} parameter(0)
  cvt_lhs = bf16[2048,64,5334]{2,1,0} convert(p_lhs)
  p_rhs = bf16[512,127,4,42,42]{4,3,2,1,0} parameter(1)

  trans = bf16[512,4,127,42,42]{4,3,2,1,0} transpose(p_rhs), dimensions={0,2,1,3,4}
  bitcast = bf16[2048,5334,42]{2,1,0} bitcast(trans)

  ROOT dot = bf16[2048,64,42]{2,1,0} dot(cvt_lhs, bitcast),
