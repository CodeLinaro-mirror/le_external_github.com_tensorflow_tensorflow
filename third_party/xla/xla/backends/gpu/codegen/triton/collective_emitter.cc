...
GetBlockLevelFusionConfigForAllReduce(const se::DeviceDescription& device_info,
  ...
  static constexpr auto round_up = [](int64_t tile_size) {
  ...
  for (int i = 0; i < rank - 1; ++i) {
    output_tile->add_sizes(round_up(output_shape.dimensions(i)));
  }
  ...
  if (rank > 0) {
    ...
    output_tile->add_sizes(round_up(tile_size));
  }
  return block_level_config;
}
}  // namespace

absl::StatusOr<std::optional<BlockLevelFusionConfig>> GetBlockLevelFusionConfig(
...
absl::StatusOr<bool> TrySetGpuFusionConfigForCollectiveOp(
    const se::DeviceDescription& device_info,
    HloFusionInstruction* fusion_instr) {
  TF_ASSIGN_OR_RETURN(const std::optional<BlockLevelFusionConfig> block_config,
                      GetBlockLevelFusionConfig(device_info, fusion_instr));
