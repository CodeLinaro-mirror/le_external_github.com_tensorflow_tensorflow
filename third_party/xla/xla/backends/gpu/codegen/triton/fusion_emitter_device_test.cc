...
using DynamicSliceTest = TritonEmitterTestWithOffsetParam;

INSTANTIATE_TEST_SUITE_P(DynamicSliceSuite, DynamicSliceTest,
                         ::testing::Values(0, 1, 10, 100),
                         ::testing::PrintToStringParamName());

TEST_P(DynamicSliceTest, OneDimension) {
...
TEST_P(DynamicSliceTest, TwoDimensions) {
  constexpr absl::string_view kHloText = R"(
HloModule m
...
TEST_P(DynamicSliceTest, NoopSlice) {
  constexpr absl::string_view kHloText = R"(
f {
  p0 = s32[64, 32] parameter(0)
  c0 = s32[] parameter(1)
...
TEST_P(DynamicSliceTest, ConstantOffset) {
  int32_t offset = GetParam();
  std::string kHloText =
...
TEST_F(TritonEmitterTest, DynamicSliceOfAdd) {
  constexpr absl::string_view kHloText = R"(
HloModule m
...
TEST_F(TritonEmitterTest, DynamicSliceSameParameter) {
  constexpr absl::string_view kHloText = R"(
HloModule m
...
TEST_F(TritonEmitterTest, DynamicSliceOfDynamicSlice) {
  constexpr absl::string_view kHloText = R"(
HloModule m
...
TEST_F(TritonEmitterTest, Slice) {
  constexpr absl::string_view kHloText = R"(
HloModule m

fused_computation {
  p0 = f32[64] parameter(0)
  ROOT r = f32[8] slice(p0), slice={[2:10]}
}

ENTRY entry_computation {
  param_0 = f32[64] parameter(0)
  ROOT fusion = f32[8] fusion(param_0), kind=kCustom,
    calls=fused_computation,
    backend_config={
      "fusion_backend_config":{
      "kind":"__triton",
...
TEST_F(TritonEmitterTest, DynamicSliceOfDot) {
  const std::string kHloText = R"(
lhs {
