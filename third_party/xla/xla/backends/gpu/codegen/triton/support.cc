...
CodegenDecision IsTritonSupportedInstructionImpl(
  ...
  // ScaledDot has a special set of supported types.
  if (instr.opcode() != HloOpcode::kScaledDot) {
    CodegenDecision input_types_are_supported = CodegenDecision(
        absl::c_all_of(instr.operands(), [&](const HloInstruction* operand) {
          ...
          return IsTritonSupportedDataType(operand_type, gpu_version);
        }));

    if (!input_types_are_supported) {
      return CodegenDecision::Forbid(
          absl::StrCat("Unsupported input data type: ",
                       input_types_are_supported.Explain()));
    }
  }
