...
TEST_P(DotTypesTest, Dot) {
  // Testing B[] = dot(A[], A[]).
  // TODO(b/393299275): Trim down the list of skipped tests.
  auto [dst_type, src_type, cc] = GetParam();
  if (src_type == F64 && dst_type != F64) {
    GTEST_SKIP() << "f64 -> !f64 is not supported.";
  }
  if (src_type == S8 && dst_type == S32) {
    GTEST_SKIP() << "s8 -> s32 is not supported.";
  }
  if (primitive_util::IsFloatingPointType(src_type) !=
      primitive_util::IsFloatingPointType(dst_type)) {
    GTEST_SKIP() << "Mixing floating point and integer types is not supported.";
  }
  if (src_type == F8E4M3FN || dst_type == F8E4M3FN) {
    if (auto* cuda_cc = std::get_if<se::CudaComputeCapability>(&cc);
        cuda_cc && *cuda_cc < se::CudaComputeCapability::Hopper()) {
      GTEST_SKIP() << "F8E4M3FN is not supported pre Hopper.";
    }
  }
  ExpectedFailMode fail_mode = ExpectedFailMode::kFail;
