...
void CheckConfig(const BackendConfig& config,
                 absl::string_view expected_target) {
  auto hlo_proto = static_cast<const HloModuleProto&>(config);
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> hlo_module,
                          CreateModuleFromProto(hlo_proto));
  EXPECT_THAT(RunFileCheck(hlo_module->ToString(), expected_target),
              IsOkAndHolds(true));
}
...
TEST_F(FissionBackendTest, GetSupportedConfigsFromCublasCustomCall) {
  ...
  EXPECT_EQ(configs.value().size(), 3);

  CheckConfig(*configs.value()[0],
              "CHECK: custom_call_target=\"__cublas$gemm\"");
  CheckConfig(*configs.value()[1],
              "CHECK: custom_call_target=\"__cublas$lt$matmul\"");
  CheckConfig(*configs.value()[2], "CHECK: calls=cutlass_gemm_with_upcast");
}
