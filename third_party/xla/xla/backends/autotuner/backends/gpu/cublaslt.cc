...
CublasLtBackend::GetSupportedConfigs(
  ...
  if (!IsSupported(instr)) {
    LOG(ERROR) << "Not a CublasLt custom call instruction.";
    return {};
  }
  ...
  auto epilogue = AsBlasLtEpilogue(backend_config.epilogue());
  if (!epilogue.ok()) {
    LOG(ERROR) << "Failed to get epilogue: " << epilogue.status();
    return {};
  }
  ...
  absl::StatusOr<std::unique_ptr<BlasLt::MatmulPlan>> plan =
      se::gpu::BlasLt::GetMatmulPlan(*stream, *gemm_config, *epilogue);
  if (!plan.ok()) {
    LOG(ERROR) << "Failed to get matmul plan: " << plan.status();
    return {};
  }

  absl::StatusOr<RedzoneBuffers> rz_buffers = RedzoneBuffers::FromInstruction(
      instr, allocator.get(), *stream, RedzoneBuffers::kAllInputsAllOutputs,
      true, true,
      instr.GetModule()
          ->config()
          .debug_options()
  ...
  if (!rz_buffers.ok()) {
    LOG(ERROR) << "Failed to get redzone buffers: " << rz_buffers.status();
    return {};
  }
  se::DeviceMemoryBase workspace_buffer =
      (*rz_buffers)
          .output_buffers()
          .at(instr.shape().tuple_shapes().size() - 1);

  absl::StatusOr<std::vector<BlasLt::MatmulAlgorithm>> algorithms =
      (*plan)->GetAlgorithms(*stream, GemmConfig::kNumAlgorithms,
                             workspace_buffer.size());
  if (!algorithms.ok()) {
    LOG(ERROR) << "Failed to get algorithms: " << algorithms.status();
    return {};
  }
