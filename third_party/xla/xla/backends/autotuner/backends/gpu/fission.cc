...
absl::StatusOr<std::unique_ptr<FissionBackendConfig>> FissionFusionToCublas(
  ...
  if (dot == nullptr) {
    return absl::InvalidArgumentError("Fusion does not contain a dot.");
  }
  ...
  if (is_rewritten_to_cublas_custom_call) {
    return std::make_unique<HloModuleProto>(hlo_module->ToProto());
  }

  return absl::InvalidArgumentError("Not supported.");
}

absl::StatusOr<std::unique_ptr<FissionBackendConfig>>
FissionFusionToCustomKernel(const HloFusionInstruction* fusion,
...
void AddConfigIfOk(
    ...
    absl::StatusOr<std::unique_ptr<HloModuleProto>> status_or_config_proto) {
  if (status_or_config_proto.ok()) {
    configs.push_back(std::move(*status_or_config_proto));
  }
}
...
FissionBackend::GetSupportedConfigs(const HloInstruction& instr,
  ...
  std::vector<std::unique_ptr<BackendConfig>> configs;
  AddConfigIfOk(configs, FissionFusionToCublas(fusion, stream_executor,
                                               /*rewrite_to_cublaslt=*/false));
  AddConfigIfOk(configs, FissionFusionToCublas(fusion, stream_executor,
                                               /*rewrite_to_cublaslt=*/true));
  AddConfigIfOk(configs, FissionFusionToCustomKernel(fusion, stream_executor));
