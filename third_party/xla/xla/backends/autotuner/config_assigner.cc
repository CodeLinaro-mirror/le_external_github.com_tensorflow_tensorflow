...
absl::Status ConfigAssigner::AssignConfigs(
  ...
  CHECK(optimal_config_cache_ != nullptr)
      << "Sharding autotuning requires a cache.";
...
tsl::Future<ConfigAssigner::Config> ConfigAssigner::GetConfig(
  ...
  if (options_.select_first_config) {
    ...
    for (Config& config : supported_configs) {
      ...
      if (executable.ok()) {
        VLOG(1) << "Using first supported config: " << config.ToString();
...
AutotunerCacheInterface::CacheStats ConfigAssigner::GetCacheStats() {
  if (optimal_config_cache_) {
