...
absl::StatusOr<std::unique_ptr<ConfigAssigner>> ConfigAssigner::Create(
  ...
  std::optional<Config> forced_config;
  if (!options.force_config.empty()) {
    autotuner::Config config_proto;
    bool parsed = tsl::protobuf::TextFormat::ParseFromString(
        options.force_config, &config_proto);
    if (!parsed) {
      parsed = config_proto.ParseFromString(options.force_config);
    }
    if (!parsed) {
      return absl::InvalidArgumentError(absl::StrCat(
          "Failed to parse force_config as textproto or binary proto: ",
