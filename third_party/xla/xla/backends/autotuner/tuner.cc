...
tsl::Future<Tuner::Config> Tuner::GetTunedConfig(const HloInstruction* instr) {
  ...
  return std::move(compile_results)
      .Map([instr,
            this](std::vector<CodegenOrchestrator::CompilationResult>&&
        ...
        ASSIGN_OR_RETURN(std::vector<ConfigResult> profile_results,
                         ProfileAll(std::move(executable_candidates), instr),
                         _ << "Autotuning failed for HLO: " << instr->ToString()
