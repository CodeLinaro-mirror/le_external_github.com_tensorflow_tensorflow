...
absl::StatusOr<XnnFusionThunk::XnnRuntime> XnnFusionThunk::CreateXnnRuntime(
  ...
  bool capturing = !captured_arguments_.empty();
...
absl::Status XnnFusionThunk::ResetXnnRuntime(
    XnnRuntime& runtime,
    absl::Span<const se::DeviceMemoryBase> arguments_buffers) {
  DCHECK(capturing_builder_) << "XNN runtime is not capturing arguments";
  DCHECK_EQ(runtime.captured_arguments.size(), captured_arguments_.size())
  ...
  if (runtime.captured_arguments == CaptureArguments(arguments_buffers)) {
    VLOG(3) << absl::StreamFormat("Reuse XNN runtime for `%s` operation",
                                  info().op_name);
    return absl::OkStatus();
  }

  VLOG(3) << absl::StreamFormat("Reset XNN runtime for `%s` operation",
                                info().op_name);
...
std::vector<se::DeviceMemoryBase> XnnFusionThunk::CaptureArguments(
    absl::Span<const se::DeviceMemoryBase> arguments_buffers) {
  std::vector<se::DeviceMemoryBase> captured_arguments;
  captured_arguments.reserve(captured_arguments_.size());
  for (int64_t i = 0; i < captured_arguments_.size(); ++i) {
    int32_t arg_index = captured_arguments_[i];
    captured_arguments.push_back(arguments_buffers[arg_index]);
  }
  ...
  auto invoke = [&](typename XnnRuntimePool::BorrowedObject runtime) {
    auto executed = runtime->Invoke(
        ...
        absl::MakeSpan(results_buffers), [&](size_t id) {
          return absl::c_linear_search(captured_arguments_, id);
        });
