...
TEST(CudaGraphTopologyMapperTest, EmptyChildGraphHandling) {
  // Test 1: Inline empty child graph (child size == 0).
  // Parent (1) size 2, Child (2) size 0.
  // Merged size should be: 2 + 0 - 1 = 1.
  {
    absl::flat_hash_map<uint32_t, size_t> base_sizes = {{1, 2}, {2, 0}};
    absl::flat_hash_map<uint32_t,
                        std::vector<CudaGraphTopologyMapper::ChildGraphEntry>>
        child_graphs;
    child_graphs[1].push_back({2, 1, /*is_conditional=*/false});
  ...
  // Test 2: Conditional empty child graph (child size == 0).
  // Parent (1) size 2, Child (2) size 0.
  // Merged size should be: 2 + 0 = 2.
  {
    absl::flat_hash_map<uint32_t, size_t> base_sizes = {{1, 2}, {2, 0}};
    absl::flat_hash_map<uint32_t,
                        std::vector<CudaGraphTopologyMapper::ChildGraphEntry>>
  ...
  // Test 3: Completely empty parent and child graphs.
  {
    absl::flat_hash_map<uint32_t, size_t> base_sizes = {{1, 0}, {2, 0}};
    absl::flat_hash_map<uint32_t,
                        std::vector<CudaGraphTopologyMapper::ChildGraphEntry>>
        child_graphs;
    child_graphs[1].push_back({2, 0, /*is_conditional=*/false});
