...
def _cc_ir_header_impl(ctx):
    ...
    objs = compilation_outputs[1].pic_objects  #or compilation_outputs[1].objects
    if not objs:
        print("WARNING: No objects found in compilation_outputs. Compilation contexts: {}".format(compilation_contexts))
        print("Compilation outputs: {}".format(compilation_outputs))
        # Fallback to a dummy file to avoid immediate crash, or let it crash to see the error.
        # For now, let's let it crash but with more info.
