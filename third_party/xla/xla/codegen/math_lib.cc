...
class IntrinsicAdapter : public MathFunction {
 public:
  ...
  std::vector<std::string> TargetFunctions() const override {
    // Find all scalar SupportedVectorTypes() and use them to generate the
    // target function names.
    std::vector<std::string> target_functions;
    for (const std::vector<Type>& types : SupportedVectorTypes()) {
      if (types[0].is_scalar()) {
        target_functions.push_back(apply_vector<Intrinsic::kNumArgs>(
