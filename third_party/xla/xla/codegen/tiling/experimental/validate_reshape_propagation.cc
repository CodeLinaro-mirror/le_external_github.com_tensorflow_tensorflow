...
absl::Status ValidateConfig(const Shape& from_shape, const Shape& to_shape,
  ...
  // false positive of negative we propagate the tile symbolically and
  ...
  Tile output_tile(*ts, std::move(output_tiles));
  ASSIGN_OR_RETURN(Tiles input_tiles,
                   PropagateTileToInput(*ts, *reshape, output_tile, 0));
  CHECK_EQ(input_tiles.size(), 1);
  Tile input_tile = input_tiles[0];
  llvm::DenseMap<SymbolicExpr, SymbolicExpr> replacement_map =
      GetTileSizeReplacementMap(*ts, config.sizes);
  RETURN_IF_ERROR(ts->AssignTileSizes(config.sizes));
  ...
  absl::Status equivalent = VerifyTileEquivalence(
      output_tile, from_shape, input_tile, to_shape, ts.get());
