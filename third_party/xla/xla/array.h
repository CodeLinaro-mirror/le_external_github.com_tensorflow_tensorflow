...
class Array {
 ...
 private:
  ...
  void TransposeDimensionsImpl3DOrMore(absl::Span<const IntT> permutation,
    ...
    absl::InlinedVector<size_t, 8> multipliers(num_dims);
    multipliers[num_dims - 1] = 1;
    for (size_t k = num_dims - 1; k > 0; --k) {
      multipliers[k - 1] = multipliers[k] * permuted.dim(k);
    }
    DCHECK_EQ(multipliers[0] * permuted.dim(0), num_elements());
    ...
    //                      permuted_dims: { 8,  9,  7}
    //                         multpliers: {63,  7,  1}
    // multipliers_aligned_with_src_array: { 1, 63,  7}
    absl::InlinedVector<size_t, 8> multipliers_aligned_with_src_array(num_dims);
    for (size_t k = 0; k < num_dims; ++k) {
      multipliers_aligned_with_src_array[permutation[k]] = multipliers[k];
    }
    ...
    const size_t src_value_index_end = num_elements() - 1;
      ...
      for (dim_index_plus_1 = num_dims;; --dim_index_plus_1) {
        ...
        if (new_index < dim(dim_index)) {
          ...
          dst_value_index = dst_value_index_stack[dim_index] +
                            multipliers_aligned_with_src_array[dim_index];
