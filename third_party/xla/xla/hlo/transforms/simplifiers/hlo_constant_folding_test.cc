...
TEST_F(HloConstantFoldingTest, FoldTuple) {
  // Create a simple while loop with zero iterations so that the default folding
  // configuration of zero iterations will still fold it.
  // TODO(willfroom): Update this test with more iterations when that is
  // configurable.
  constexpr absl::string_view mod_str = R"(
    HloModule test
    condition_fn
    {
      ...
      ROOT compare.1 = pred[] compare(index, s32[] constant(0)), direction=LT
    }
