...
namespace {
bool IsAssociative(HloComputation* comp) {
  ...
  HloOpcode opc = root->operand(0)->opcode();
  return opc == HloOpcode::kAdd || opc == HloOpcode::kMultiply ||
         opc == HloOpcode::kMinimum || opc == HloOpcode::kMaximum;
}
}  // namespace
