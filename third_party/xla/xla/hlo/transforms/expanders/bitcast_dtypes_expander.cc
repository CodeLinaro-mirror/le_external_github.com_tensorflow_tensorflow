...
absl::StatusOr<Shape> CollapseLastDimension(const Shape& shape) {
  if (shape.dimensions().size() < 2) {
    return absl::InvalidArgumentError("Shape has less than 2 dimensions.");
  }
  std::vector<int64_t> collapsed_dimensions(shape.dimensions().begin(),
                                            shape.dimensions().end() - 1);
  collapsed_dimensions.back() *= shape.dimensions().back();
  return ShapeUtil::MakeShape(shape.element_type(), collapsed_dimensions);
}
...
absl::StatusOr<HloInstruction*> Upcast(HloInstruction* input,
  ...
  ASSIGN_OR_RETURN(Shape collapsed_shape, CollapseLastDimension(input_shape));
  ASSIGN_OR_RETURN(HloInstruction * collapsed_input,
                   MakeReshapeHlo(collapsed_shape, input));

  // Initialize start and limit indices for slicing.
  std::vector<int64_t> input_dimensions(input_shape.dimensions().begin(),
                                        input_shape.dimensions().end());
  std::vector<int64_t> start_indices(input_rank - 1, 0);
  std::vector<int64_t> strides(input_rank - 1, 1);
  strides.back() = input_shape.dimensions().back();
  std::vector<int64_t> limit_indices(collapsed_shape.dimensions().begin(),
                                     collapsed_shape.dimensions().end());
  limit_indices.back() -= input_shape.dimensions().back();
