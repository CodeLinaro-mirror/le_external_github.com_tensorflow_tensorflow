...
const LogicalBuffer& TuplePointsToAnalysis::GetBuffer(
  ...
  auto buffer_or = logical_buffer_analysis_->GetBuffer(id);
  CHECK(buffer_or.ok()) << buffer_or.status().ToString();
  return *buffer_or.value();
}
...
bool TuplePointsToAnalysis::DoesNotUseOperandBuffer(
  ...
  if (user->IsLoopFusion()) {
    ...
    auto buffer_or = GetBufferDefinedAt(*it, index);
    CHECK(buffer_or.ok()) << buffer_or.status().ToString();
