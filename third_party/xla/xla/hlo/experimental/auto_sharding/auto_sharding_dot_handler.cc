...
absl::Status DotHandler::RegisterStrategies() {
  auto split_func = [&](const DimMap& output_dim_map) {
  ...
  EnumerateGeneral(split_func, ins_->shape().rank(), /*current_tensor_dim=*/0,
                   all_mesh_dims, /*current_dim_map=*/{});
  SortStrategies();
...
void ConvHandler::GenerateConvolutionShardingStrategiesFromOutputSharding(
  ...
  for (int64_t mesh_dim : unused_mesh_dims) {
    ...
    std::function<double(const HloSharding&)> communication_cost_fn =
