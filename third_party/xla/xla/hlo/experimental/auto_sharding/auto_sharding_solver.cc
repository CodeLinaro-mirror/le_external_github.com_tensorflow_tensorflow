...
AutoShardingSolverResult SolveAndExtractSolution(
  ...
  if (request.memory_budget() > 0) {
    ...
    if (!request.deterministic_mode()) {
      peak_times.insert(request.peak_times().begin(),
                        request.peak_times().end());
    }
    // Also add in any time points that have a relatively small number of terms.
    for (LivenessIdx time_idx = 0; time_idx < request.live_size(); ++time_idx) {
      if (request.live(time_idx).nodes_size() <= kMemoryCardinalityThreshold) {
        peak_times.insert(time_idx);
      }
    }
    // Impose memory constraints at each of these peak times.
    for (const LivenessIdx peak_time_idx : peak_times) {
