...
bool RequiresCustomHloConverter(const llvm::ListInit* custom_hlo_converter_ops,
                                const llvm::Record* def) {
  for (const auto* my_op : custom_hlo_converter_ops->getValues()) {
    if (my_op->getAsString() == def->getName()) return true;
  }
  return false;
}
...
static bool OperatorWritersMain(raw_ostream& os, const RecordKeeper& records) {
  ...
  const auto* custom_hlo_converter_ops = llvm::dyn_cast_or_null<llvm::ListInit>(
      records.getGlobal("CustomHloConverterOps"));
  if (!custom_hlo_converter_ops) {
  ...
  for (const auto* def : records.getAllDerivedDefinitions("MHLO_Op")) {
    ...
    if (!RequiresCustomHloConverter(custom_hlo_converter_ops, def))
      BuildOperator(op, os);
  }
