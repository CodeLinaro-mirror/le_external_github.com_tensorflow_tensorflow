...
TEST(MeshAndAxisTest, Overlaps_SubAxes_Overlap) {
  ...
  EXPECT_TRUE(AxisRef(1, {2, 8}).Overlaps(AxisRef(1, {4, 2})));
}
...
TEST(MeshAndAxisTest, GetPrefixWithoutOverlap_CannotCoexist) {
  EXPECT_EQ(AxisRef(0, {1, 2}).GetPrefixWithoutOverlap(AxisRef(0, {1, 3})),
            std::nullopt);
  EXPECT_EQ(AxisRef(0, {1, 3}).GetPrefixWithoutOverlap(AxisRef(0, {2, 3})),
            std::nullopt);
}
...
TEST(MeshAndAxisTest, SortAndMergeAxesNoMerge) {
  Mesh mesh({16}, {"x"});
  std::vector<AxisRef> axes = {AxisRef(0, {1, 2}), AxisRef(0, {4, 2})};
  SortAndMergeAxes(axes, mesh);

  EXPECT_THAT(axes,
              testing::ElementsAre(AxisRef(0, {1, 2}), AxisRef(0, {4, 2})));
}

TEST(MeshAndAxisTest, SortAndMergeAxes_DifferentMeshAxes) {
  Mesh mesh({4, 4}, {"x", "y"});
  std::vector<AxisRef> axes = {AxisRef(0, {1, 2}), AxisRef(1, {1, 2})};
  SortAndMergeAxes(axes, mesh);
  EXPECT_THAT(axes,
              testing::ElementsAre(AxisRef(0, {1, 2}), AxisRef(1, {1, 2})));
}
