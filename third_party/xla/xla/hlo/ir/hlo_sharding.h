...
class HloSharding {
 public:
  ...
  NamedSharding::ReductionOp reduction_op() const { return reduction_op_; }
  void set_reduction_op(NamedSharding::ReductionOp op) {
    reduction_op_ = op;
 ...
 private:
  void ExtractReductionOpFromMetadata() {
    for (const auto& md : metadata_) {
      if (md.op_type() == "sdy::reduction_op") {
        if (md.op_name() == "MAX") {
          reduction_op_ = NamedSharding::ReductionOp::kMax;
        } else if (md.op_name() == "MIN") {
          reduction_op_ = NamedSharding::ReductionOp::kMin;
        } else {
          reduction_op_ = NamedSharding::ReductionOp::kSum;
        }
      }
    }
  }
  ...
  NamedSharding::ReductionOp reduction_op_ = NamedSharding::ReductionOp::kSum;
