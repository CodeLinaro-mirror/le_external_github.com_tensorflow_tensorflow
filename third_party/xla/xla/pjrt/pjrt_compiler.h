...
class PjRtTopologyDescription {
 public:
  ...
  // given chip.
  virtual absl::StatusOr<std::pair<PjRtProcessId, int>> ProcessIdForChip(
      PjRtGlobalChipId chip_id) const {
    return absl::UnimplementedError("ProcessIdForChip is unsupported.");
  }

  // Returns the process ID and the index on process for a logical device.
  virtual absl::StatusOr<std::pair<PjRtProcessId, int>>
  ProcessIdForLogicalDeviceOfDefaultType(
      xla::PjRtGlobalDeviceId device_id) const {
    return absl::UnimplementedError(
        "ProcessIdForLogicalDeviceOfDefaultType is unsupported.");
  }
