...
TEST(PjRtCApiClientTpuTest, ClearMemoryStats) {
  ...
  EXPECT_GE(stats_after_alloc.bytes_in_use,
            initial_stats.bytes_in_use + kAllocSize);
  EXPECT_GE(stats_after_alloc.peak_bytes_in_use,
            initial_stats.peak_bytes_in_use + kAllocSize);

  buffer.reset();

  TF_ASSERT_OK_AND_ASSIGN(auto stats_after_reset, device->GetAllocatorStats());
  EXPECT_EQ(stats_after_reset.bytes_in_use, initial_stats.bytes_in_use);
  EXPECT_GE(stats_after_reset.peak_bytes_in_use,
            initial_stats.peak_bytes_in_use + kAllocSize);
