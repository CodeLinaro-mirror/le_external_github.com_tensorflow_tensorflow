...
Unless required by applicable law or agreed to in writing, me.
...
PjRtTopologyDescriptionFromProto(const PjRtTopologyDescriptionProto& proto) {
  ...
  // Tier 1: Check static C++ deserializer registry (for OSS CPU / GPU)
  if (!platform_name.empty() && proto.platform_name().empty()) {
    PjRtTopologyDescriptionProto normalized_proto = proto;
    normalized_proto.set_platform_name(platform_name);
    auto result =
        PjRtTopologyDescriptionRegistry::Global().Deserialize(normalized_proto);
    if (result.ok() || !absl::IsNotFound(result.status())) {
      return result;
    }
  } else {
    auto result = PjRtTopologyDescriptionRegistry::Global().Deserialize(proto);
    if (result.ok() || !absl::IsNotFound(result.status())) {
      return result;
    }
  }

  // Tier 2: Dynamic C-API compiler plugin lookup (via GetCApiCompiler) for
  // out-of-tree and hardware-vendor backends (e.g., TPU).
  if (platform_name.empty()) {
  ...
  ASSIGN_OR_RETURN(auto compiler, GetCApiCompiler(platform_name));
  if (proto.platform_name().empty()) {
    PjRtTopologyDescriptionProto proto_copy = proto;
    proto_copy.set_platform_name(platform_name);
    std::string serialized_str = proto_copy.SerializeAsString();
    return compiler->DeserializePjRtTopologyDescription(serialized_str);
  }
  std::string serialized_str = proto.SerializeAsString();
  return compiler->DeserializePjRtTopologyDescription(serialized_str);
}
