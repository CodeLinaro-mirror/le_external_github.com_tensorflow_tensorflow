...
// TODO(cobley) - add tests for other collectives too.
...
TEST(GlooCollectives, AllReduce) {
  ...
  // Reserve a vector of output buffers with one buffer per participant.
  std::vector<absl::StatusOr<std::vector<uint8_t>>> output_buffers;
  output_buffers.reserve(kNumParticipants);
