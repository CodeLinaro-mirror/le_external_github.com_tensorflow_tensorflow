...
// A collection of buffers and a RedZone allocator that store the inputs and
...
class RedzoneBuffers {
 public:
  enum BuffersToCreate {
    ...
    // is considered a scratch buffer. If the result shape is not a tuple, this
    // will create a buffer corresponding to the entire shape - equivalent to
    // `kAllInputsAllOutputs`.
