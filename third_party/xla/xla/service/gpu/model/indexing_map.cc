...
void IndexingMap::CompressVars(const llvm::SmallBitVector& unused_dims,
                               const llvm::SmallBitVector& unused_symbols) {
  std::vector<DimVar> compressed_dim_vars;
  MLIRContext* mlir_context = GetMLIRContext();

  // Compress dimensions in affine map.
  unsigned num_dims_before = GetDimensionCount();
  if (!unused_dims.empty()) {
    affine_map_ = mlir::compressDims(affine_map_, unused_dims);
  }
  bool num_dims_changed = num_dims_before > GetDimensionCount();

  // Compress symbols in affine map.
  unsigned num_symbols_before = GetSymbolCount();
  if (!unused_symbols.empty()) {
    affine_map_ = mlir::compressSymbols(affine_map_, unused_symbols);
  }
  bool num_symbols_changed = num_symbols_before > GetSymbolCount();

  if (!num_dims_changed && !num_symbols_changed) return;
...
SmallBitVector IndexingMap::RemoveUnusedSymbols() {
  ...
  CompressVars(/*unused_dims=*/{}, unused_vars.unused_symbols);
...
SmallBitVector IndexingMap::RemoveUnusedDimensions() {
  ...
  CompressVars(unused_vars.unused_dims, /*unused_symbols=*/{});
...
SmallBitVector IndexingMap::RemoveUnusedVars() {
  ...
  CompressVars(unused_vars.unused_dims, unused_vars.unused_symbols);
  return std::move(unused_vars.unused_symbols);
}
