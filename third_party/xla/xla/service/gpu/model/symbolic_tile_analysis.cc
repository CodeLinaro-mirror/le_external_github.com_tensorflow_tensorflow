...
/*static*/ SymbolicTileAnalysisOrError SymbolicTileAnalysis::AnalyzeFusion(
  ...
  get_tiled_hlo_instruction =
    ...
    bool is_parameter = !fusion.ContainsInstruction(instruction_adaptor);

    tiled_hlo_instructions.push_back(
        std::make_unique<SymbolicTiledHloInstruction>(
            hlo, is_parameter, std::move(indexing_map),
            std::move(*symbolic_tile)));
  ...
  get_tiled_hlo_instruction =
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        TiledHloInstruction::Create(
            symbolic_tiled_hlo->hlo(), symbolic_tiled_hlo->is_parameter(),
