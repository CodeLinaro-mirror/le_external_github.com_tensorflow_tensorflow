...
FusionDecision ShouldProceedWithSymbolicTileDerivation(
  ...
  if (hlo->opcode() == HloOpcode::kDot ||
    ...
    return FusionDecision::Deny("Bailing out on ") << hlo->ToString();
  }
  ...
  if (hlo->opcode() == HloOpcode::kReshape ||
    ...
    if (!reshape_symbolic_tile.has_value()) {
      return FusionDecision::Deny("Bailing out on reshape ")
...
SetSymbolicTilesAndComputeConstraints(
  ...
  for (const std::unique_ptr<SymbolicTiledHloInstruction>&
    ...
    if (!symbolic_tile.has_value()) {
      return FusionDecision::Deny("Failed to compute symbolic tile for ")
             << indexing_map.ToString() << " for HLO " << hlo->ToString();
    }

    if (!symbolic_tile->is_satisfiable()) {
      return FusionDecision::Deny("Symbolic tile ")
    ...
    if (!constraints.is_satisfiable()) {
      return FusionDecision::Deny("Fusion has unsatisfiable constraints");
    }
...
/*static*/ SymbolicTileAnalysisOrError SymbolicTileAnalysis::AnalyzeFusion(
  ...
  if (roots.size() > 1) {
    return FusionDecision::Deny("Multi-output fusions are not supported. ")
           << fusion.ToString();
  }
  auto& root = roots[0];
