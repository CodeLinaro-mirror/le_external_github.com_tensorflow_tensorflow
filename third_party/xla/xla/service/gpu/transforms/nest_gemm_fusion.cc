...
absl::Status AnnotateDotOperandNestedFusion(
...
absl::Status AnnotateDotLhsNestedFusion(HloFusionInstruction& nested_fusion,
  ...
  return AnnotateDotOperandNestedFusion(
...
// Finds a tile size for the root of the analysis that satisfies the
// requirements of the dot. That is, the tile sizes needs to satisfy the
// constraints of the anlysis and map to the given config of the dot.
absl::StatusOr<llvm::SmallVector<int64_t>> FindOutputTileSizesForEpilogue(
  ...
  // Try all permutations of the fot tile sizes to see if any of them satisfy
  ...
  do {
    ...
    if (!parameters_satisfy_constraints) continue;
...
class NestGemmFusionsVisitor : public DfsHloRewriteVisitor {
 public:
  explicit NestGemmFusionsVisitor(mlir::MLIRContext* ctx) : ctx_(ctx) {}
  ...
  absl::Status HandleFusion(HloInstruction* instruction) override {
    ...
    for (HloInstruction* instruction : computation->instructions()) {
      ...
      if (dot != nullptr) {
        return absl::InternalError(
            "The fusion's called computation has more than one dot.");
      }
      dot = Cast<HloDotInstruction>(instruction);
    }
    if (dot == nullptr) {
      return absl::InternalError(
          "The fusion's called computation doesn't have a dot.");
    }
    ...
    TF_RETURN_IF_ERROR(MakeNestedFusionFromGemmFusion(fusion, config, analysis,
                                                      **tiled_dot_it, dot));
    this->MarkAsChanged();
    return absl::OkStatus();
  }
...
absl::StatusOr<bool> NestGemmFusion::Run(
  ...
  for (HloComputation* computation :
    ...
    NestGemmFusionsVisitor visitor(&ctx);
