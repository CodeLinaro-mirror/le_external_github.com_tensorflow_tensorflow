...
HloInstruction* AsType(HloInstruction* instruction, PrimitiveType type) {
...
HloInstruction* ReduceDimensions(HloInstruction* instruction,
  ...
  return computation->AddInstruction(HloInstruction::CreateReduce(
      reduce_shape, AsType(instruction, accumulator_type), zero, reduce_dims,
      add_computation));
}
...
absl::StatusOr<HloInstruction*> DotStrengthReduction::ExpandInstruction(
  ...
  flow = AsType(flow, dot->shape().element_type());
  return flow;
}
