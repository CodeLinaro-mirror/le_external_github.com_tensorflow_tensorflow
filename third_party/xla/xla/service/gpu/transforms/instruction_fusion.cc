...
FusionDecision GpuInstructionFusion::ShouldFuseInexpensiveChecks(
  ...
  if (producer->opcode() == HloOpcode::kFusion) {
    return FusionDecision::Deny("the producer is a fusion");
  }

  if (consumer->IsCustomFusion()) {
    return FusionDecision::Deny("the consumer is a custom fusion");
  }
  ...
  if (CreatesHeavyComputation(*producer, *consumer)) {
    return FusionDecision::Deny("the fusion would create a heavy computation");
  }
