 ...
 public:
  ...
  void OnFusingInstruction(HloInstruction* fusion,
    ...
    fusion_deduplication_cache_.UpdateFusedInstructionId(
        ...
        original_consumer_operand_index,
        /*use_multi_output_fusion=*/creates_multi_output_fusion);
    ...
    for (HloInstruction* operand : fusion->operands()) {
      ...
      // so we can do incremental update of the operand. If `operand` had a
      // preferred consumer, no incremental update is needed (but we may need to
      // do a full update if the preferred consumer was `original_consumer`).
      if (it == preferred_consumer_.end()) {
        operands_to_new_consumers_[operand].push_back(fusion);
      } else if (it->second == original_consumer) {
 ...
 private:
  ...
  Priority CalculateProducerPriority(HloInstruction* producer) {
    ...
    if (auto fusion_decision = CanFuseWithAllNonBitcastUsers(producer);
      ...
      if (CanFuseTritonMultiOutputWithSingleNonBitcastUser(producer,
                                                           &consumer)) {
        ...
        preferred_consumer_[producer] = consumer;
  ...
  FusionDecision OperandReachableFromProducer(const HloInstruction* producer,
                                              const HloInstruction* consumer) {
    for (const auto* operand : consumer->operands()) {
      CHECK(reachability_->IsPresent(operand) &&
      ...
      if (producer != operand &&
        ...
        return FusionDecision::Forbid(absl::StrCat(
            producer->name(), " would introduce a cycle when fused"));
      }
    }
    return FusionDecision::Allow();
  }

  FusionDecision CanFuseTritonMultiOutputWithSingleNonBitcastUser(
    ...
    return OperandReachableFromProducer(producer, *consumer);
  }
...
absl::StatusOr<bool> PriorityFusion::Run(
  ...
  for (auto* computation : fusible_computations) {
    ...
    while (fusion_queue->DequeueNextProducer()) {
      ...
      bool use_multi_output_fusion = false;
      if (preferred_consumer.has_value()) {
      ...
      if (use_multi_output_fusion || producer->user_count() == 0) {
        ...
        // have been removed already if it has no users anymore.
