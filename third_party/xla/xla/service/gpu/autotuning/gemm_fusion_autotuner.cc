...
absl::Status RewriteGemmFusionToCustomKernelFusion(
  ...
  pipeline.AddPass<CustomKernelFusionRewriter>(&device_description,
                                               kernel_index);
  HloModule* hlo_module = fusion_instr->GetModule();
  TF_RETURN_IF_ERROR(pipeline.Run(hlo_module).status());

  return absl::OkStatus();
}

absl::Status GemmFusionAutotunerConfigGeneratorVisitor::HandleFusion(
...
absl::StatusOr<bool> GemmFusionAutotuner::Run(
  ...
  return GemmFusionAutotunerConfigGeneratorVisitor(config_).RunOnModule(
      module, execution_threads);
}
