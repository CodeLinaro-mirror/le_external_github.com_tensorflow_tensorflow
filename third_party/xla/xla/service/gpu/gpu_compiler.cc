...
bool RequiresCollectiveInput(HloModule* module, const HloUse& use,
...
bool RequiresCollectiveOutput(HloModule* module, const HloInstruction* def,
                              const DebugOptions& opts) {
  const bool is_nccl_buffers_used =
      opts.xla_gpu_enable_nccl_user_buffers() ||
      opts.xla_gpu_experimental_enable_nccl_symmetric_buffers();
...
void GpuCollectiveBufferAnalysis(
  ...
  for (const HloBuffer& buffer : alias_analysis.buffers()) {
    const HloValue* entry_input_value = nullptr;
    ...
    for (const HloValue* value : buffer.values()) {
      ...
      if (RequiresCollectiveOutput(module, def, opts)) {
    ...
    VLOG(5) << "  entry_input_value: "
            << (entry_input_value == nullptr
                    ? "null"
                    : entry_input_value->ToShortString());
    VLOG(5) << "  live_out_value: "
            << (live_out_value == nullptr ? "null"
    ...
    if (entry_input_value != nullptr && used_by_collective) {
      VLOG(2)
          << "Special Copy Insertion Case A: Entry input is used by operation "
             "that requires collective memory space operands. Inserting copy "
             "for "
          << entry_input_value->ToShortString();
      add_index_to_copy(entry_input_value->defining_instruction(),
                        entry_input_value->defining_index());
    }

    // Special Copy Insertion Case B: Entry output
    if (defined_by_collective && live_out_value != nullptr) {
      VLOG(2) << "Special Copy Insertion Case B: Collective output flows "
                 "directly to Entry Output. "
              << "Searching for entry-level ROOT position for "
              << live_out_value->ToShortString();
      ...
      if (!marked_for_copy) {
        LOG(WARNING)
            << "Special Copy Insertion Case B: Could not find entry-level ROOT "
               "position for collective output in buffer "
            << buffer.id();
      }
    }
  }
