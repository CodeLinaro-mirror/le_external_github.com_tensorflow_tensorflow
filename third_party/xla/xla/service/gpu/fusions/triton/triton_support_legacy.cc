...
CodegenDecision IsInstructionSupportsDataTypes(
  ...
  if (!IsTritonSupportedDataType(instr.shape().element_type(), gpu_version)) {
    return CodegenDecision::Deny("Unsupported output data type.");
  }
    ...
    switch (instr.opcode()) {
      ...
      default:
        if (!IsTritonSupportedDataType(operand_type, gpu_version)) {
          return CodegenDecision::Deny("Unsupported input data type.");
        }
    }
  }
...
CodegenDecision CanTritonHandleElementwise(
  ...
  if (instr.opcode() == HloOpcode::kConstant) {
    ...
    return CodegenDecision::Deny("Unsupported elementwise operation.");
  }
  return CodegenDecision::Allow();
}
...
CodegenDecision CanTritonHandleGEMM(
  ...
  if (!IsTritonSupportedDotOutputType(dot.shape().element_type(),
                                      gpu_version)) {
    return CodegenDecision::Deny("Unsupported output data type for Dot op.");
  }
  ...
  if (!IsTritonSupportedDataType(dot.operand(0)->shape().element_type(),
      ...
      !IsTritonSupportedDataType(dot.operand(1)->shape().element_type(),
                                 gpu_version)) {
    return CodegenDecision::Deny("Unsupported input data type for Dot op.");
  }
  ...
  if (dim_numbers.lhs_batch_dimensions().size() > 1) {
    return CodegenDecision::Deny("Multiple batch dimensions.");
  }
...
CodegenDecision IsTritonSupportedInstruction(
  ...
  switch (instr.opcode()) {
    case HloOpcode::kDot: {
      ...
      if (NoNonContractingDimension(*dot)) {
        return CodegenDecision::Deny("No non-contracting dimensions.");
      }
    ...
    case HloOpcode::kTuple: {
      ...
      return CodegenDecision::Deny("Only supports root tuples.");
    }
  ...
  return CodegenDecision::Deny("Unsupported opcode.");
}
