...
Value ApplyMask(ImplicitLocOpBuilder& b, int64_t dim, Value input, int denom,
                Value k, int64_t dims_k, int64_t block_k, Value pid_k) {
  auto c32 = [&](int64_t v) { return CreateConst(b, b.getI32Type(), v); };
...
absl::Status EmitMatMul(mlir::OpBuilder builder,
  ...
  auto body_builder = [&](mlir::OpBuilder&, mlir::Location, Value ki,
    ...
    if (need_masking) {
      dot_input_lhs = ApplyMask(b, 0, dot_input_lhs, is_sparse ? 2 : 1, ki,
                                dims.k, block_k, scopes.pid_k());
      dot_input_rhs = ApplyMask(b, 1, dot_input_rhs, 1, ki, dims.k, block_k,
                                scopes.pid_k());
