...
// This contains all alternative GEMM configs related to one fusion.
using GemmConfigSet = std::vector<GemmFusionAutotunerImpl::Config>;
  ...
  absl::Status HandleFusion(const HloInstruction* hlo) override {
    ...
    bool missing_config = (backend_config.kind() == kTritonGemmFusionKind &&
                          ...
                          (backend_config.kind() == kCuDnnFusionKind &&
                           !backend_config.has_cudnn_fusion_config());
    if (missing_config) {
      TF_ASSIGN_OR_RETURN(GemmConfigSet configs,
...
AutotuneResult FromConfig(const GemmFusionAutotunerImpl::Config& config) {
  AutotuneResult res;
...
absl::Status Autotune(
    GemmFusionAutotunerImpl& impl, AutotunerCompileUtil& util,
    const GemmFusionAutotunerImpl::CompilationTask& gemm_config_sets) {
  TF_ASSIGN_OR_RETURN(
      GemmFusionAutotunerImpl::CompilationResult executable_sets,
      impl.CompileAll(util, gemm_config_sets));
  ...
  for (const auto& key_value : executable_sets) {
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        impl.Profile(util, *fusion, /*candidates=*/key_value.second));

    if (!impl.GetDebugOptions().xla_gpu_cublas_fallback() &&
    ...
    if (impl.GetDebugOptions().xla_gpu_dump_autotuned_triton_fusions()) {
      TF_RETURN_IF_ERROR(DumpAutotunedFusion(impl.GetConfig(), util, best,
                                             fusion, fusion_id++));
    }

    const AutotuneCacheKey key =
        AutotunerUtil::GetKey(fusion, impl.GetConfig());
...
absl::StatusOr<GemmFusionAutotunerImpl::CompilationResult>
          ...
          for (int num_warps : kNumWarps) {
            ...
            for (int split_k : kSplitK) {
              ...
              for (int num_ctas : kNumCtas) {
                ...
                if (!cc.IsAtLeastHopper() && num_ctas > 1) {
                  break;
                }
...
absl::StatusOr<bool> GemmFusionAutotuner::Run(
  ...
  if (!autotuner.IsAutotuningEnabled()) {
    ...
    TF_RETURN_IF_ERROR(
        Autotune(autotuner, *opt_compile_util, gemm_config_sets));
