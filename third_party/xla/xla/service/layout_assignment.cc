...
absl::Status LayoutAssignment::AddCallConstraints(HloInstruction* instruction) {
  if (computation_layouts_.find(instruction->to_apply()) ==
...
absl::Status LayoutAssignment::ConstrainAsyncOperands(
    HloInstruction* instruction, int first_operand_idx, int start_param_idx,
    const ComputationLayout& async_layout) {
  int num_operands_to_constrain =
      instruction->operand_count() - first_operand_idx;
  for (int i = 0; i < num_operands_to_constrain; ++i) {
    int op_idx = first_operand_idx + i;
    int param_idx = start_param_idx + i;
    if (param_idx < async_layout.parameter_count() &&
        async_layout.parameter_layout(param_idx).LayoutIsSet()) {
      RETURN_IF_ERROR(SetOperandLayout(
          async_layout.parameter_layout(param_idx).shape(), instruction, op_idx,
          /*mandatory=*/true, /*dfs=*/true));
    }
  }
  return absl::OkStatus();
...
absl::Status LayoutAssignment::AddAsyncOpResultLayoutConstraint(
...
absl::Status LayoutAssignment::AddAsyncStartConstraints(
  ...
  RETURN_IF_ERROR(ConstrainAsyncOperands(instruction, /*first_operand_idx=*/0,
                                         /*start_param_idx=*/0, async_layout));
  return AddAsyncOpResultLayoutConstraint(instruction, async_layout,
                                          async_constraint);
}
...
absl::Status LayoutAssignment::AddAsyncUpdateConstraints(
  ...
  // Determine how many parameters were bound before this async-update.
  const HloInstruction* prev_async_op = instruction->operand(0);
  int prev_bound_parameters =
      prev_async_op->shape().tuple_shapes(0).tuple_shapes_size();

  RETURN_IF_ERROR(ConstrainAsyncOperands(
      instruction, /*first_operand_idx=*/1,
      /*start_param_idx=*/prev_bound_parameters, async_layout));
  return AddAsyncOpResultLayoutConstraint(instruction, async_layout,
                                          async_constraint);
}
