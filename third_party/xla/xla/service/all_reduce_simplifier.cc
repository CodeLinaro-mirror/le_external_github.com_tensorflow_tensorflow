...
absl::StatusOr<bool> AllReduceSimplifier::Run(
  ...
  for (auto computation : module->computations(execution_threads)) {
    for (HloInstruction* inst : computation->MakeInstructionPostOrder()) {
      ...
      // The replication analysis performed earlier is only for cross replica
      // spmd. So if the AllReduce is not cross replica, we will only simplify
      // it if the group size is 1 (only a single device is involved).
      if (group_size == -1 ||
