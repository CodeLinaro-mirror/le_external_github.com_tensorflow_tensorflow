...
std::unique_ptr<tsl::protobuf::Message> ProcessCustomDescInFallbackTest(
  ...
  if (msg_dynamic) {
    const proto2::Reflection* refl = msg_dynamic->GetReflection();
    const proto2::Descriptor* d = msg_dynamic->GetDescriptor();
    const proto2::FieldDescriptor* f = d->FindFieldByName("some_flag");
...
TEST_F(CompilationEnvironmentsTest, GetEnvTriggersFullNameFallback) {
  ...
  const proto2::Descriptor* desc_generated =
      test::TestCompilationEnvironment1::descriptor();
  proto2::DescriptorPool custom_pool(proto2::DescriptorPool::generated_pool());
  proto2::FileDescriptorProto file_proto;
  ...
  const proto2::Descriptor* desc_custom =
