...
HloSharding convertToHloSharding(
  ...
  SmallVector<AxisRefAttr> manualAxesAxisRef;
  llvm::transform(manualAxes, std::back_inserter(manualAxesAxisRef),
                  [&](StringRef manualAxis) {
                    return AxisRefAttr::get(context, manualAxis);
                  });

  // Iterate the manual axes.
  if (!manualAxesAxisRef.empty()) {
    types.push_back(OpSharding::MANUAL);
    int64_t& manualDim = tileAssignmentDims.emplace_back(1);
    for (AxisRefAttr axisRef : manualAxesAxisRef) {
