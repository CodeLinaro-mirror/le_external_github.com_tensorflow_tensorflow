...
class ManualComputationPattern
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    SmallVector<StringAttr> parentManualAxes = parentManualCompAxes.lookup(op);
  ...
  void runOnOperation() final {
    ...
    module->walk<mlir::WalkOrder::PreOrder>([&](ManualComputationOp op) {
      if (auto parentOp = op->getParentOfType<ManualComputationOp>()) {
        parentManualCompAxes[op] = parentManualCompAxes[parentOp];
        parentManualCompAxes[op].insert(parentManualCompAxes[op].end(),
                                        parentOp.getManualAxes().begin(),
