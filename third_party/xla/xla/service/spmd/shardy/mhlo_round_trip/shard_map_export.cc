...
class ManualComputationPattern
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    auto convertToAxisRef = [&](StringRef manualAxis) {
    ...
    llvm::transform(manualCompAxes.at(op), std::back_inserter(parentManualAxes),
                    convertToAxisRef);
 ...
 private:
  void runOnOperation() final {
    ...
    module->walk<mlir::WalkOrder::PreOrder>([&](ManualComputationOp op) {
      assert(!manualCompAxes.contains(op));
      if (auto parentOp = op->getParentOfType<ManualComputationOp>()) {
        manualCompAxes[op] = manualCompAxes[parentOp];
        manualCompAxes[op].append(
            SmallVector<StringAttr>(parentOp.getManualAxes()));
      } else {
        manualCompAxes[op] = {};
      }
    });
