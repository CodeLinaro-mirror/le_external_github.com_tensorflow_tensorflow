...
absl::StatusOr<bool> UnstableReductionDetector::Run(
  ...
  // We don't care about the return value. It is only used for testing.
  CheckModule(module);
  return false;
}

UnstableReductionDetector::Result UnstableReductionDetector::CheckModule(
  ...
  if (module->config().debug_options().xla_detect_unstable_reductions() ==
    ...
    return Result::kSkipped;
  }
  for (const HloInstruction* reduction : unstable_reductions) {
    LOG(WARNING) << "Unstable reduction found in module '" << module->name()
                 << "': " << reduction->ToString();
  }
  if (!unstable_reductions.empty() &&
      module->config().debug_options().xla_detect_unstable_reductions() ==
          DebugOptions::UNSTABLE_REDUCTION_DETECTION_MODE_CRASH) {
    LOG(FATAL) << "Unstable reductions found in module '" << module->name()
               << "'";
  }
  if (unstable_reductions.empty()) {
    return Result::kNoProblemsFound;
  }
  return Result::kUnstableReductionsFound;
}
