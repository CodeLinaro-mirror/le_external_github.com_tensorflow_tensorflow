...
// unnecessary copy of the accumulation buffer in the outer body. We see an
// example of jax.scan program and its hlo version below: def cumsum(res, elem):
//     """
...
//  body {
//   param = (s32[], s32[], s32[8]) parameter(0)
//   i = s32[] gte(param), index=0
//   init = s32[] gte(param), index=1
//   carry = s32[8] gte(param), index=2
...
//   new_value = f(dynamic-slice, init) // f is the cumsum function
//   dynamic-update-slice = s32[8] dynamic-update-slice(carry, new_value, i)
//   ROOT tuple.10 = (s32[], s32[], s32[8]) tuple(i++, add.1,
