...
absl::StatusOr<Shape> DotOperandDims::ComputeOutputShape(
  ...
  ASSIGN_OR_RETURN(Shape output_shape, ShapeUtil::MakeValidatedShape(
...
absl::StatusOr<DotOperandDims> DotOperandDims::ApplyTransformationsFrom(
  ...
  int64_t num_batch = src_after.Rank(kBatch);
  int64_t num_contracting = src_after.Rank(kContracting);
  int64_t num_nc = Rank(kNonContracting);
  int64_t total_rank = num_batch + num_contracting + num_nc;

  auto check_bounds = [&](absl::Span<const int64_t> indices, const char* msg) {
    if (absl::c_any_of(indices, [&](int64_t dim) {
          return dim < 0 || dim >= total_rank;
        })) {
  ...
  std::vector<bool> is_taken(total_rank, false);
  for (Category cat : {kBatch, kContracting}) {
  ...
  available_positions.reserve(num_nc);
  ...
  if (available_positions.size() != num_nc) {
    return absl::InternalError(
        "Failed to determine available positions for non-contracting "
        "dimensions");
  }

  std::vector<int64_t> target_dimensions(total_rank);
  ...
  fill_target(available_positions, Sizes(kNonContracting));
