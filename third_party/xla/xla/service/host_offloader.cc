...
absl::StatusOr<bool> HostOffloader::HandleRedundantCopiesBackToHost(
  ...
  for (size_t index = 0; index < done_shape.tuple_shapes_size(); index++) {
    ...
    ShapeIndex start_shape_index = {1, static_cast<int64_t>(index)};
    std::queue<InstructionAndShapeIndex> queue;
    queue.push(InstructionAndShapeIndex(call_done, output_shape_index));

    // Add both async-start (def) and async-done (use)
    // TODO(b/347101407): Start from async-start and trace through the
    // computation as well in GetSuccessors.
    ...
    while (!queue.empty() && host_only) {
      ...
      TF_ASSIGN_OR_RETURN(std::vector<InstructionAndShapeIndex> successors,
                          GetSuccessors(InstructionAndShapeIndex(
      ...
      for (InstructionAndShapeIndex& successor : successors) {
        // We conservatively only consider a few cases which can accept host
        // memory space.
        if (IsValidDuringPureMemoryOffload(successor.instruction) &&
            successor.instruction->opcode() != HloOpcode::kAsyncStart &&
            successor.instruction->opcode() != HloOpcode::kAsyncDone) {
    ...
    for (HloInstruction* instruction : computation->instructions()) {
      if (IsHostAsyncStart(instruction)) {
        TF_ASSIGN_OR_RETURN(changed_in_loop,
                            HandleRedundantCopiesBackToHost(instruction));
