  ...
  if (reserved_allocations_it !=
    ...
    for (std::unique_ptr<ReservedAllocation>& reserved_allocation_ptr :
      ...
      if (inclusive_start_time == definition_time &&
        ...
        properties.request_segment_for_output_colored_in_alt_mem = true;
      ...
      if (reserved_allocation_ptr->end_time() == use_time) {
        properties.use_colored_in_alternate_memory = true;
        ReleaseReservedAllocationForAlternateMemoryColorings(
            reserved_allocation_ptr.get());
      }
    }
  ...
  if (default_memory_colorings_it !=
    ...
    for (int64_t coloring_time : default_memory_colorings_it->second) {
      if (inclusive_start_time == definition_time &&
        ...
        properties.request_segment_for_output_colored_in_default_memory = true;
      }
      if (coloring_time == use_time) {
        properties.use_colored_in_default_memory = true;
      }
    }
  }
  return properties;
...
AllocationResult MsaAlgorithm::AllocateSegment(AllocationRequest& request) {
  ...
  if (!colored_allocation_request_properties
           .request_segment_for_output_colored_in_default_memory &&
      !colored_allocation_request_properties.use_colored_in_default_memory &&
  ...
  if (colored_allocation_request_properties
          .request_segment_for_output_colored_in_alt_mem) {
  ...
  if (!request.allocation_value->requires_contiguous_allocation()) {
    ...
    CHECK(!colored_allocation_request_properties
               .request_segment_for_output_colored_in_default_memory);
    CHECK(!colored_allocation_request_properties.use_colored_in_default_memory);
    return result_mark(AllocationResult::kFailRequiresUncommit,
                       allocation_result);
  }
  ...
  if (request.allow_prefetch &&
    ...
    AllocationResult prefetch_result = Prefetch(
        request, **prev_allocation_in_default_mem_it, nullptr,
        /*force_prefetch=*/
        colored_allocation_request_properties.use_colored_in_alternate_memory);
  ...
  CHECK(!colored_allocation_request_properties.use_colored_in_alternate_memory);
