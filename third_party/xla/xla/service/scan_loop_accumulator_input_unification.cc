...
FindAccumulatorInputPairs(const HloAliasAnalysis& alias_analysis,
  ...
  for (int64_t param_idx = 0;
    ...
    for (HloInstruction* gte : body_param->users()) {
      ...
      // The accumulator should only be used exactly once.
  ...
  for (HloInstruction* acc : possible_acc) {
    ...
    VLOG(3) << "acc index: " << idx << " = " << acc_gte_outer_body->name();
    if (idx == -1) {
      continue;
    }
    ...
    VLOG(3) << "input_gte_outer = " << input_gte_outer->name();
    ...
    VLOG(3) << "input_gte_inner = " << input_gte_inner->name();

    // Check if the input_gte_inner is a shape covering read-only instruction
    if (MatchDynamicSliceInDim(input_gte_inner->users().at(0), config)) {
...
// Given a list of unollable loops and their config, finds all the
...
absl::StatusOr<bool> UnifyAccumulatorWithInput(
  ...
  auto is_while_body = [&](HloComputation* comp) {
    ...
    return callers.size() > 0 && callers.at(0)->opcode() == HloOpcode::kWhile;
  };
