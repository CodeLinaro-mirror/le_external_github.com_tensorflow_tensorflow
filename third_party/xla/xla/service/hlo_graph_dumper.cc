...
std::string HloDotDumper::GetInstructionNodeExtraInfo(
  ...
  if (caller.has_value() && instr->opcode() == HloOpcode::kParameter) {
    const auto* operand = caller.value()->operand(instr->parameter_number());
    const auto* producer = GetInterestingProducer(operand);
    lines.push_back(
        StrFormat("from %s in %s", HtmlLikeStringSanitize(producer->name()),
