...
bool IsAcfPrameter(const xla::HloInstruction* instruction) {
  ...
  if (parent_computation->name().starts_with(kAcfComputationName)) {
    return gteOperand->name().starts_with(kAcsInstructionName) &&
           instruction->users()[0] == parent_computation->root_instruction();
  } else if (fusion_instruction->name().starts_with(kAcdInstructionName)) {
    // Parameter is fused into AsyncCollectiveDone, mapped from Params in
    // AsyncCollectiveFusion - operand is gte from ACF
    return gteOperand->fused_instructions_computation()->name().starts_with(
        kAcfComputationName);
  }
  return false;
}
