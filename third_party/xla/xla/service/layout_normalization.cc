...
class LayoutNormalizationVisitor : public DfsHloRewriteVisitor {
 public:
  ...
  Status HandleScatter(HloInstruction* hlo) override {
    ...
    auto operand_permutation = InversePermutation(ToTransposeDimensions(
        scatter->scatter_operands()[0]->shape().layout()));
    ...
    std::vector<int64_t> normalized_scatter_dims_to_operand_dims;
    normalized_scatter_dims_to_operand_dims.reserve(
        dims.scatter_dims_to_operand_dims_size());
    for (int64_t dim : dims.scatter_dims_to_operand_dims()) {
      normalized_scatter_dims_to_operand_dims.push_back(
          operand_permutation[dim]);
    }
    std::copy(normalized_scatter_dims_to_operand_dims.begin(),
              normalized_scatter_dims_to_operand_dims.end(),
              tsl::protobuf::RepeatedFieldBackInserter(
                  normalized_dims.mutable_scatter_dims_to_operand_dims()));
    ...
    std::copy(normalized_update_window_dims.begin(),
              normalized_update_window_dims.end(),
              tsl::protobuf::RepeatedFieldBackInserter(
                  normalized_dims.mutable_update_window_dims()));
    std::vector<int64_t> normalized_inserted_window_dims;
    normalized_inserted_window_dims.reserve(dims.inserted_window_dims_size());
    for (int64_t dim : dims.inserted_window_dims()) {
      normalized_inserted_window_dims.push_back(update_permutation[dim]);
    }
    std::copy(normalized_inserted_window_dims.begin(),
              normalized_inserted_window_dims.end(),
              tsl::protobuf::RepeatedFieldBackInserter(
                  normalized_dims.mutable_inserted_window_dims()));

    // Note that after layout normalization, the scatter might not be simplified
    // anymore, so we need to run it through ScatterSimplifier pass again.
