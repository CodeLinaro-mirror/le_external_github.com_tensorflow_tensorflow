    ...
    switch (user->opcode()) {
      case HloOpcode::kFusion: {
        for (int64_t idx : user->OperandIndices(hlo)) {
          ...
          if (fusion_is_simple &&
              fusion_users[0]->opcode() == HloOpcode::kSlice) {
            size += handle_slice(user, fusion_users[0]);
          } else if (fusion_is_simple &&
                     fusion_users[0]->opcode() == HloOpcode::kDynamicSlice) {
            size += handle_dynamic_slice(user, fusion_users[0]);
          } else if (fusion_is_simple && fusion_users[0]->opcode() ==
                                             HloOpcode::kDynamicUpdateSlice) {
            size += handle_dynamic_update_slice(user, fusion_users[0]);
          } else {
            auto nested_size =
