...
TEST(FfiTest, RecordStageCreateAndUpdateCommands) {
  ...
  XLA_FFI_RecordApi dummy_record_api = {
      /*create_launch=*/+[](XLA_FFI_RecordContext* ctx, const char*,
                            const void*, size_t, XLA_FFI_SourceFormat,
      ...
      /*create_memcpy_d2d=*/
      +[](XLA_FFI_RecordContext* ctx, void*, void*, size_t,
          const XLA_FFI_Command* const*, uint32_t,
  ...
  auto handler = Ffi::BindRecord().Ctx<RecordContext>().To(
  ...
  // Create a record.
  {
    XLA_FFI_RecordFrame record_frame = {
        ...
        &num_commands,
        /*max_commands=*/10};
    RecordExtension record_frame_ext = BuildRecordExtension(&record_frame);
