...
CudaExecutor::CreateDeviceDescription(int device_ordinal) {
  ...
  if (absl::StatusOr<nvmlDevice_t> status_or_device = GetNvmlDevice(pci_bus_id);
      status_or_device.ok()) {
    absl::StatusOr<int64_t> status_or_bandwidth =
        GetDevicePcieBandwidth(*status_or_device);
    ...
    absl::StatusOr<int64_t> status_or_p2p_link_count =
        GetNumberOfActiveP2PNvlinks(*status_or_device);
    ...
    auto status_or_fabric_info = GetDeviceFabricInfo(*status_or_device);
    if (status_or_fabric_info.ok()) {
      info.cluster_uuid = status_or_fabric_info->cluster_uuid;
      info.clique_id = status_or_fabric_info->clique_id;
    } else {
