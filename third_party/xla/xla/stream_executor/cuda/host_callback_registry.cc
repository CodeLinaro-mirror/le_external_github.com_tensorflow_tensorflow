...
absl::Status HostCallbackRegistry::AddCallback(
  ...
  if (const absl::Status status = enqueue_cb(kCudaCallback, node.get());
      !status.ok()) {
    std::move(node)->Cancel(status);
    return status;
  }
  AppendNode(node.release());  // Hand off ownership.
  // On the off chance that the registry is shutting down now the destructor
  // will cancel all remaining callbacks.
...
void HostCallbackRegistry::AppendNode(HostCallbackNode* node) {
  // prev is guaranteed to be non-null because of the sentinel.
  HostCallbackNode* prev = tail_.exchange(node, std::memory_order_acq_rel);
