...
absl::StatusOr<TmaDescriptor> TmaDescriptor::FromProto(
  ...
  // will a more thorough validation.
  ...
  absl::InlinedVector<uint64_t, 5> global_dims(proto.global_dims().begin(),
                                               proto.global_dims().end());
  ...
  absl::InlinedVector<uint64_t, 5> global_strides(
  ...
  absl::InlinedVector<uint32_t, 5> box_dims(proto.box_dims().begin(),
                                            proto.box_dims().end());
  ...
  absl::InlinedVector<uint32_t, 5> element_strides(
...
TmaMetadataProto TmaMetadata::ToProto() const {
  ...
  for (const auto& [arg_index, tma_info] : arg_index_to_tma_info) {
    (*proto.mutable_arg_index_to_tma_info())[arg_index] = tma_info.ToProto();
  }
  return proto;
}
...
absl::StatusOr<TmaMetadata> TmaMetadata::FromProto(
  ...
  for (const auto& [arg_index, tma_info] : proto.arg_index_to_tma_info()) {
    ...
    metadata.arg_index_to_tma_info.emplace(arg_index, std::move(descriptor));
  }
  return metadata;
}
