...
TEST(SingleArgumentPackingSpecTest, WriteArgumentAddress) {
  ...
  // Since we are referencing argument #2, we will need to provide 3 arguments.
  first_arg.WriteArgumentAddress(/*argument_index=*/2);

  // We fail if not enough arguments are provided.
  EXPECT_THAT(first_arg.BuildArgument(
                  {DeviceMemoryBase(nullptr, 0), DeviceMemoryBase(nullptr, 0)}),
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ...
      first_arg.BuildArgument(
          {DeviceMemoryBase(
               absl::bit_cast<void*>(static_cast<uintptr_t>(0xff42)), 0),
           DeviceMemoryBase(
               absl::bit_cast<void*>(static_cast<uintptr_t>(0xaabbccdd)), 0)}));
  EXPECT_THAT(first_arg_storage,
