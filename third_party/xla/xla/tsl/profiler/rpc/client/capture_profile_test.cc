...
TEST(CaptureProfileTest, ExportToTensorBoardVectorXSpacesEmpty) {
  ...
  EXPECT_OK(ExportToTensorBoard(empty_xspaces, temp_dir, "run"));
  EXPECT_OK(ExportToTensorBoard(empty_xspaces, temp_dir));
}
...
TEST(CaptureProfileTest, ExportToTensorBoardVectorXSpacesFailurePropagation) {
  ...
  EXPECT_THAT(ExportToTensorBoard(xspaces, invalid_logdir, "run"), Not(IsOk()));
}
