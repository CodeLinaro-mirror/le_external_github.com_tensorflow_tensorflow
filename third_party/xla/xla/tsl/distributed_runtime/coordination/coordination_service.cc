...
class CoordinationServiceStandaloneImpl : public CoordinationServiceInterface {
 ...
 private:
  ...
  // Validate that the barrier is invoked with the right args. Returns false if
  ...
  bool ValidateBarrierArgs(
      ...
      const std::vector<CoordinatedTask>& participating_tasks,
      StatusCallback done);
  // Initialize a new barrier. Returns false if the barrier should fail
  // immediately.
...
void CoordinationServiceStandaloneImpl::BarrierAsync(
  ...
  if (inserted) {
    bool successful_init = InitializeBarrier(barrier, barrier_id, timeout, task,
